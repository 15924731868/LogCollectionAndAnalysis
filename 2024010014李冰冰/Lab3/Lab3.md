...
> 记录：本次使用的 Ubuntu 用户名为 ___user123___。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.225.128_。
...
> 记录：本机主机名为 ____user123-VMware-Virtual-Platform__（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 | 2026-09-19T16:09:29.891879+08:00 | user123 | 192.168.225.1 | Accepted password | /var/log/auth.log |
| 失败认证 | 2026-09-19T16:10:53.505934+08:00 | user123 | 192.168.225.1 | Failed password | /var/log/auth.log |
...
| :--- | :--- |
| journal 中是否查到 | 可以查到 |
| `/var/log/syslog` 中是否查到 | 可以查到 |
| 两处记录有哪些共同字段或正文 | 正文完全一致，标签lab3_read，包含student_id=2024010014 name=李冰冰 action=write_test result=success |
| 两处输出的主要区别 | journalctl附带PID、日志优先级等journald元数据；syslog文本日志格式简洁，只有时间、主机名、标签、消息正文，元数据更少 |
...
```text
获取命令：sudo grep -F "student_id=2024010014" /var/log/syslog | tail -n 5
日志原文：2026-09-19T16:51:06.155830+08:00 user123-VMware-Virtual-Platform lab3_read: student_id=2024010014 name=李冰冰 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026‑09‑19T16:51:06.155830+08:00 |
| Where 在哪里 |主机 user123‑VMware‑Virtual‑Platform |
| Who 谁 | 标签`lab3_read`；消息标识学号 2024010014，姓名李冰冰|
| What 做了什么 |使用 logger 向本机日志系统写入一条 write_test 测试消息 |
| Result 结果如何 | 正文标记`result=success`，为手动写入标记；journal 与 syslog 两处均可检索到这条消息|

**用一两句话解释这个事件：**

> 填写：2026‑09‑19 16:51:06，在 user123‑VMware‑Virtual‑Platform 主机上执行 logger 写入自定义测试日志。`result=success`是人为写进去的文本标记，不是系统给出的执行结果，这条消息同时被 journald 与 rsyslog 捕获
...
日志原文：2026-09-19T16:10:53.505934+08:00 user123-VMware-Virtual-Platform sshd[4582]: Failed password for user123 from 192.168.225.1 port 51060 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026‑09‑19T16:10:53.505934+08:00 |
| Where 在哪里 | 主机 user123‑VMware‑Virtual‑Platform|
| Who 谁 |记录程序 sshd，进程号 4582；尝试登录账号 user123；客户端来源 IP192.168.225.1 |
| What 做了什么 |客户端通过 SSH 协议尝试使用 user123 账号密码登录虚拟机|
| Result 结果如何 | `Failed password`，本次密码认证失败|

**用一两句话解释这个事件：**

> 填写：2026‑09‑19 16:10:53，来自 192.168.225.1 的客户端发起 SSH 登录，对账号 user123 进行密码校验，密码不匹配，认证失败，事件记录在`/var/log/auth.log`。
...
日志原文：2026-09-19 16:58:07 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-19 16:58:07；该日志未提供时区|
| Where 在哪里 |该日志未提供主机名；记录取自 Ubuntu 虚拟机`/var/log/dpkg.log` |
| Who 谁 |记录工具 dpkg；该日志未提供执行操作的用户账号 |
| What 做了什么 | 记录 htop 软件包安装完成后的状态信息|
| Result 结果如何 |状态`installed`，htop 软件包已成功安装；仅凭本行不能区分首次安装还是升级 |

**用一两句话解释这个事件：**

> 填写：dpkg 软件包管理器记录 htop 软件包状态为已安装，该日志行没有记录执行本次安装操作的用户。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 的日志守护进程，接收系统、程序输出的日志，保存为二进制 journal，供 journalctl 读取。rsyslog 接收 journald 转发过来的日志，将日志持久化保存为文本文件，如 syslog、auth.log。同一条消息交给 journald 之后，journald 一方面存入自身二进制 journal，另一方面转发这条消息交给 rsyslog 写入文本日志，所以 journal 和 syslog 能够同时保存同一条事件。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：能证明：有来自对应 IP 的 SSH 连接，使用指定账号尝试密码登录，本次密码校验失败，sshd 记录下时间、账号、来源 IP。不能证明：无法确定操作客户端的真实人员，不能直接判定是黑客攻击，单凭这一行日志无法知道密码输错的根本原因。
