...
> 记录：Ubuntu 的日期和时间为 __2026-09-17 15:30:54____；时区为 __Asia/Shanghai____；时间同步状态为 _已同步_____。
...
> 记录：`/var/log/syslog` 是否存在？__存在____；实际输出：__syslog exists____。
...
> 记录：`/var/log/auth.log` 是否存在？__存在____；实际输出：__auth.log exists____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.118.128____。
...
> 记录：22 端口是否正在监听？__是____；输出中的本地地址和端口为 __**0.0.0.0:22、[::]:22**____。
...
> 记录：SSH 登录后的主机名为 __ubuntu-VMware-Virtual-Platform____。
...
> 记录：当前目录为 __/home/ubuntu____；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：`/var/log/journal/` 的检查结果为 __存在____；`/run/log/journal/` 的检查结果为 __存在____。
...
| :--- | :--- | :--- | :--- |
| /var/log/syslog|系统全局通用日志，记录大部分系统事件 |文本文件 |cat / less |
|/var/log/auth.log |认证、登录、sudo 权限相关安全日志 |文本文件 |cat / less |
|/var/log/wtmp |记录用户登录 / 登出历史	 |二进制数据文件 |last |
|/var/log/lastlog |记录每个用户最近一次登录信息 |二进制数据文件 |lastlog |
|/var/log/journal |systemd 持久化 journal 日志存放目录 |目录 |journalctl |
...
> 记录：选取的一条日志来自哪个程序或服务？__sudo____；记录了什么事件？__ubuntu 用户执行 sudo 命令，打开 root 会话，执行 journalctl 查询日志____。
...
> 记录：实际查询起点为 __2026-09-17 16:10:00____；终点为 __2026-09-17 16:20:00____；观察到的事件或无记录情况为 __sysstat-collect 系统统计服务启停、用户执行 sudo 调用 journalctl 查询日志、NetworkManager 网络管理服务分配 IP 并启动调度服务____。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 __kernel: [Firmware Bug]: TSC doesn't count with P0 frequency!____。
...
> 填写：`/var/log/wtmp` 和 `/var/log/btmp` 是二进制数据库文件，不是纯文本，cat 读取只会显示乱码。查看`wtmp`用`last`命令，查看`btmp`用`lastb`命令。
