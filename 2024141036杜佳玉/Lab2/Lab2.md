...
> 记录：rsyslog 的实际运行状态为 ___active___。
...
> 记录：Ubuntu 的日期和时间为 __Wed 2026-09-16 16:48:51 CST____；时区为 __Asia/Shanghai____；时间同步状态为 ___yes___。
...
> 记录：`/var/log/syslog` 是否存在？__存在____；实际输出：___syslog exists___。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ___192.168.150.128___。
...
> 记录：当前目录为 ___/home/dujiayu___；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：`/var/log/journal/` 的检查结果为 __目录存在____；`/run/log/journal/` 的检查结果为 __目录存在____。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog|系统综合服务日志|文本|sudo tail /var/log/syslog|
|/var/log/auth.log|SSH、sudo、用户登录认证日志|文本|sudo tail /var/log/auth.log|
|/var/log/wtmp|用户登录 / 注销历史记录|二进制数据文件|last|
|/var/log/btmp|失败登录记录|二进制数据文件|sudo lastb|
|/var/log/journal/|systemd 持久化 journal 二进制日志|目录（存放二进制日志）|journalctl|
...
> 记录：选取的一条日志来自哪个程序或服务？__systemd____；记录了什么事件？___启动用户会话，加载系统服务___。
...
> 记录：实际查询起点为 __2026-09-16 16:00:00____；终点为 __2026-09-16 18:00:00____；观察到的事件或无记录情况为 __查询到多条日志，包含 gnome-shell 窗口警告、cron 定时任务执行、sudo 用户登录会话记录____。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 __gnome-shell[2406]: Window manager warning: Ping serial 1217230 was reused for window W3, previous use was for window W17____。
...
> 填写：`/var/log/wtmp` 和 `/var/log/btmp` 是**二进制文件**，不是纯文本，cat 读取只会显示乱码。查看 wtmp 用`last`命令，查看 btmp 用`lastb`命令。
