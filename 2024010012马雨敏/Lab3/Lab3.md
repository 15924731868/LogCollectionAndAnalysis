...
> 记录：本次使用的 Ubuntu 用户名为 ____mym__。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 ____192.168.125.128 __。
...
> 记录：本机主机名为 ____mym-VMware-Virtual-Platform__（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026‑09‑17T08:14:17.082095+08:00 |mym |192.168.125.1 | Accepted password| /var/log/auth.log|
| 失败认证 |2026‑09‑17T08:14:12.562945+08:00 |mym |192.168.125.1 |Failed password |/var/log/auth.log |
...
| 两处记录有哪些共同字段或正文 |时间戳、主机名、标签`lab3_read`；日志内容完全一致：`student_id=2024010012 name=马雨敏 action=write_test result=success` |
| 两处输出的主要区别 |journalctl 输出是简易日期格式（月 日 时：分: 秒），syslog 输出是带微秒的 ISO 完整时间戳；journalctl 会显示进程 PID`[4267]`，syslog 没有 PID 字段；journal 是 systemd 二进制日志，syslog 为文本磁盘日志文件。 |
...
```text
获取命令：sudo grep -F "student_id=2024010012" /var/log/syslog | tail -n 5
日志原文：2026-09-17T08:27:26.203574+08:00 mym-VMware-Virtual-Platform lab3_read: student_id=2024010012 name=马雨敏 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |026‑09‑17T08:27:26.203574+08:00 |
| Where 在哪里 | mym‑VMware‑Virtual‑Platform|
| Who 谁 | 自己指定的标签 lab3_read 写入；正文中标识本人姓名为马雨敏，学号 2024010012|
| What 做了什么 |向本机日志系统写入一条 write_test 测试消息|
| Result 结果如何 | journal 与 syslog 两处查询均可以查到该日志记录，完成验证|

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日 08:27:26，本人在 mym‑VMware‑Virtual‑Platform 主机上使用 logger 命令写入一条学号 2024010012 的测试日志消息，正文标记结果为 success。
...
日志原文：2026-09-17T08:14:17.082095+08:00 mym‑VMware‑Virtual‑Platform sshd[3487]: Accepted password for mym from 192.168.125.1 port 65473 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026‑09‑17T08:14:17.082095+08:00|
| Where 在哪里 |本机主机 mym‑VMware‑Virtual‑Platform；客户端来源 IP：192.168.125.1 |
| Who 谁 |客户端以用户 mym 账号尝试 SSH 登录服务器 |
| What 做了什么 | 客户端通过 ssh 协议向 Ubuntu 虚拟机发起远程登录认证|
| Result 结果如何 |密码校验通过，SSH 登录成功 |

**用一两句话解释这个事件：**

> 填写：2026‑09‑17 08:14:17，IP 为 192.168.125.1 的设备使用 mym 账号访问虚拟机，密码验证正确，SSH 远程登录成功。
...
日志原文：2026-09-17 08:35:21 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026‑09‑17 08:35:21；该日志没有给出时区信息|
| Where 在哪里 |日志本身不带主机名；记录取自 Ubuntu 虚拟机文件 `/var/log/dpkg.log` |
| Who 谁 |由 dpkg 软件包工具生成这条记录，日志中看不到是哪个系统用户执行安装 |
| What 做了什么 |记录 htop 软件包的安装状态变更事件 |
| Result 结果如何 |状态标记为 installed，代表软件包已经完成安装；单凭这一行分不清是首次安装还是升级 |

**用一两句话解释这个事件：**

> 填写：2026‑09‑17 08:35:21，系统 dpkg 工具记录 htop 软件包已经处于已安装状态，该日志行没有记录执行该操作的用户账号。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd‑journald 是 systemd 自带日志服务，收集系统全部日志，以二进制格式存储；rsyslog 接收日志，把日志保存为普通文本日志文件（如 `/var/log/syslog`、`/var/log/auth.log`）。journald 收到日志后，除了自己保存，还会把一份日志副本转发给 rsyslog，因此两者可以同时保留同一条日志记录。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：`Failed password`可以证明来自该 IP 的客户端确实向本机 mym 账号提交过密码登录尝试，并且密码校验失败。但它不能证明输入的是什么密码，也不能证明操作人的真实身份，无法区分是本人输错密码还是他人暴力尝试破解账号。
