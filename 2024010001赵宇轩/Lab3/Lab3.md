...
> 记录：本次使用的 Ubuntu 用户名为 __zyx1____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 _192.168.34.130 
_____。
...
> 记录：本机主机名为 __zyx1-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |	9月22日 14:35:23	 |zyx1 | 192.168.34.1	|Accepted password for zyx1	 |	journalctl 系统日志	 |
| 失败认证 |9月22日 14:36:42	 |zyx1 | 192.168.34.100| 	Failed password for zyx1	|journalctl 系统日志	
 |
...
| 两处记录有哪些共同字段或正文 |标签lab3_read，student_id=2024010001 name=zyx1 action=write_test result=success，主机名相同|
| 两处输出的主要区别 |journal显示简写时间+进程PID；syslog是带毫秒与时区的完整ISO时间，无PID |
...
```text
获取命令：sudo grep -a -F "student_id=2024010001" /var/log/syslog | tail -n 5
日志原文：2026-09-23T15:25:02.800145+08:00 zyx1-VMware-Virtual-Platform lab3_read: student_id=2024010001 name=zyx1 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-23 15:25:02
 |
| Where 在哪里 |zyx1-VMware-Virtual-Platform（本机Ubuntu） |
| Who 谁 |lab3_read
 |
| What 做了什么 |使用logger写入一条结构化测试日志 |
| Result 结果如何 |success，日志成功写入syslog |

**用一两句话解释这个事件：**

> 填写：在2026年09月23日15:25:02，zyx1在Ubuntu虚拟机通过logger命令向系统syslog写入一条带学号、姓名的结构化测试日志，日志写入成功。
...
```text
获取命令：sudo grep -a -E "Accepted|Failed password|sudo" /var/log/auth.log | tail -n 30
日志原文：2026-09-23T16:12:10.653307+08:00 zyx1-VMware-Virtual-Platform sudo: pam_unix(sshd:auth): authentication failure; logname= uid=0 euid=0 tty=pts/0 ruser=zyx1 rhost= user=zyx1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月23日 16:12:10；该显示未提供年份和时区 |
| Where 在哪里 |主机 zyx1-VMware-Virtual-Platform |
| Who 谁 |记录程序sshd；尝试登录账号zyx1 |
| What 做了什么 |通过SSH使用密码尝试登录本机 |
| Result 结果如何 |authentication failure，密码认证失败 |

**用一两句话解释这个事件：**

> 填写：9月23日16:12:10，本人在zyx1-VMware-Virtual-Platform主机上尝试通过SSH登录，账号zyx1密码认证失败。
...
日志原文：2026-09-23 16:31:06 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026年9月23日 16:31:06；该日志记录了本次htop安装完成的时间 |
| Where 在哪里 | 主机 zyx1-VMware-Virtual-Platform，日志文件路径 /var/log/dpkg.log|
| Who 谁 |记录工具dpkg；本次操作用户为zyx1|
| What 做了什么 |记录htop软件包安装完成的状态信息 |
| Result 结果如何 |状态为installed，htop软件包成功安装到系统 |

**用一两句话解释这个事件：**

> 填写：2026年9月23日16:31:06，在zyx1-VMware-Virtual-Platform虚拟机中完成htop软件安装，dpkg日志记录htop状态为installed，代表本次安装操作成功。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald是系统原生日志服务，收集内核、程序产生的日志，存放在二进制journal日志；rsyslog是传统日志服务，接收日志并输出为文本格式日志文件（如auth.log）。systemd-journald作为日志接收前端，会把日志转发给rsyslog，因此同一条日志会被两个服务分别保存。


2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password可以证明有人使用该账号尝试SSH登录，并且密码输入错误，认证失败。但无法证明登录尝试者是谁，也不能证明来源IP对应的就是操作者本人，只能说明存在一次密码错误的登录尝试。
