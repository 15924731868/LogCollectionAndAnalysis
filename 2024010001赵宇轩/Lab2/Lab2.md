...
> 记录：rsyslog 的实际运行状态为 __active ____。
...
> 记录：Ubuntu 的日期和时间为 ___2026-09-17 08:30:49__；时区为 __Asia/Shanghai___；时间同步状态为 __no____。
...
> 记录：`/var/log/syslog` 是否存在？_是_____；实际输出：_syslog exists_____。
...
> 记录：`/var/log/auth.log` 是否存在？_是____；实际输出：_auth.log exists_____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.34.128____。
...
> 记录：SSH 服务的实际运行状态为 _active_____。
...
> 记录：22 端口是否正在监听？__是____；输出中的本地地址和端口为 __ 0.0.0.0:22、[::]:22____。
...
> 记录：SSH 登录后的主机名为 ___zyx1-VMware-Virtual-Platform
___。
...
> 记录：当前目录为 _/home/zyx1
_____；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：`/var/log/journal/` 的检查结果为 ___drwxr-sr-x+ 3 root systemd-journal 4096  8月 27 09:54 /var/log/journal___；`/run/log/journal/` 的检查结果为 ___drwxr-sr-x+ 2 root systemd-journal   40  9月 17 09:13 /run/log/journal
__。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog|系统运行通用日志	 |文本 |less |
|/var/log/auth.log |用户登录、权限认证日志 |文本 |less |
|/var/log/wtmp |用户登录历史 | 二进制| last|
|/var/log/btmp |登录失败记录	 | 二进制| lastb|
| /var/log/journal/|systemd持久化系统日志库	 |目录 |journalctl	 |
**先读取文本日志**
...
> 记录：选取的一条日志来自哪个程序或服务？__NetworkManager____；记录了什么事件？__网络管理服务检测到 DHCPv4 地址变更，获取到新的 IPv4 地址 192.168.108.34____。
...
> 记录：实际查询起点为 __2026-09-17 08:00:00
____；终点为 _2026-09-17 12:00:00
_____；观察到的事件或无记录情况为 __系统启动 apt‑daily‑upgrade、dpkg‑db‑backup 等日常定时任务；fwupd‑refresh.service 执行失败；检测到网卡 ens33 链路断开____。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 _gtk_widget_get_scale_factor: assertion 'GTK_IS_WIDGET (widget)' failed_____。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制文件，不是纯文本，cat 只能读文本，直接查看会显示乱码。读取 wtmp 使用 last 命令，读取 btmp 使用 lastb 命令
