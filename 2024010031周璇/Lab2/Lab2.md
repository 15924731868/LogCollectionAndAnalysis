...
> 记录：Ubuntu 的日期和时间为 _2026-09-10 09:18:40 CST
_____；时区为 ___Asia/Shanghai (CST, +0800)
___；时间同步状态为 __yes____。
...
> 记录：`/var/log/syslog` 是否存在？__是____；实际输出：___auth.log exists
___。
...
> 记录：`/var/log/auth.log` 是否存在？____是__；实际输出：__th.log exists"
auth.log exists____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ___192.168.240.128___。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 __ISTEN 0      4096         0.0.0.0:22 ____。
...
> 记录：SSH 登录后的主机名为 _zx
zx@zx-VMware-Virtual-Platform:~$ hostname
zx-VMware-Virtual-Platform
zx@zx-VMware-Virtual-Platform:~$ pwd
_____。
...
> 记录：当前目录为 __/home/zx
____；是否确认已通过 Git Bash 登录 Ubuntu？___是___。
...
> 记录：`/var/log/journal/` 的检查结果为 _drwxr-sr-x+ 3 root systemd-journal 4096  8月 27 09:36 /var/log/journal_____；`/run/log/journal/` 的检查结果为 __drwxr-sr-x+ 2 root systemd-journal   40  9月 16 13:01 /run/log/journal
____。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog |系统核心全局日志，记录系统服务、应用程序、内核的通用运行事件 |文本文件（UTF-8 编码） |cat / tail / less |
|/var/log/auth.log | 认证安全日志，记录用户登录、sudo 权限申请、SSH 连接验证等所有身份授权相关事件|文本文件 | cat / tail / grep|
|/var/log/wtmp |二进制登录历史文件，持久化记录所有用户的登录、登出记录，以及系统重启 |二进制文件（utmp 格式） | last|
| /var/log/btmp|失败登录审计日志，专门记录所有登录失败的尝试 |二进制文件 |sudo lastb |
|/var/log/kern.log |内核专属日志，记录系统启动时内核加载、硬件驱动运行、内核异常告警等内核层面事件 |文本文件 |cat / tail / dmesg |
...
> 记录：选取的一条日志来自哪个程序或服务？_sudo（进程号 2986）_____；记录了什么事件？_**普通用户 zx 在 pts/0 终端执行 sudo 命令，申请以 root 身份运行 journalctl 日志查看命令，pam_unix 认证模块成功为该操作打开了 root 用户的权限会话**_____。
...
> 记录：实际查询起点为 _ince '2026-09-16 13:15:00_____；终点为 __until '2026-09-16 14:00:00____；观察到的事件或无记录情况为 __有____。
...
> 记录：是否查到匹配日志？___是___；其中一条的内容或无记录提示为 ____gfileinfo.c: line 1765 (g_file_info_get_icon): should not be reached
__。
...
按仓库 README 的流程单独提交 Lab2，PR 标题为 `[学号姓名]Lab2作业提交`。

---

## 七、截止时间

**2026 年 9 月 17 日 23:59:59（北京时间）**

请在截止前创建 Lab2 的 PR 并完成最后一次推送。按仓库 `README.md` 第 4 节的规则，9 月 18 日 00:00 起新建 PR 或向已有 PR 推送修改均算作超时；审核未通过的同学也应在截止前完成修改。
