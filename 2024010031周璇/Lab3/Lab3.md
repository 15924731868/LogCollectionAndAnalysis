...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.240.128____。
...
> 记录：本机主机名为 __zx-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-17 08:54:08 |zx |192.168.240.1 |Accepted password |`/var/log/auth.log`（同步记录于 systemd journal） |
| 失败认证 |2026-09-17 08:54:02 |zx |192.168.240.1 |Failed password |`/var/log/auth.log`（同步记录于 systemd journal） |
...
| 两处记录有哪些共同字段或正文 | |主机名 `zx-VMware-Virtual-Platform`、进程标识 `lab3_read[3777]`、完整日志正文 `student_id=2024010031 name=周璇 action=write_test result=success`、日志发生时间完全一致
| 两处输出的主要区别 | |1. 时间格式不同：journalctl 输出为简化的「月 日 时：分: 秒」格式，syslog 为带时区的完整 ISO 时间戳；2. 存储形式不同：journal 是 systemd 管理的二进制日志，syslog 是 rsyslog 写入的纯文本文件；
...
```text
获取命令：logger -p user.notice -t lab3_read "student_id=2024010031 name=周璇 action=write_test result=success"

日志原文：9月 17 09:01:58 zx-VMware-Virtual-Platform lab3_read[3777]: student_id=2024010031 name=周璇 action=write_test result=success

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | |9 月 17 日 09:01:58；journalctl 输出未显示完整年份，对应 syslog 完整时间为 2026-09-17T09:01:58.886225+08:00
| Where 在哪里 | |主机 `zx-VMware-Virtual-Platform`
| Who 谁 | |`lab3_read`（进程号 3777）写入；正文标识本人姓名为周璇，学号 2024010031
| What 做了什么 | |向本机日志系统写入一条 `write_test` 测试消息
| Result 结果如何 | |正文写明 `result=success`，是自定义写入的测试标记；该记录在 `journalctl` 与 `/var/log/syslog` 两处均可查询到，验证日志写入成功

**用一两句话解释这个事件：**

> 填写：9 月 17 日 09:01:58，本人在 zx-VMware-Virtual-Platform 主机上通过 logger 工具写入一条包含个人学号、姓名的自定义测试日志，标记操作结果为 success，日志成功写入系统日志体系
...
日志原文：2026-09-17T08:54:08.820116+08:00 zx-VMware-Virtual-Platform sshd[2927]: Accepted password for zx from 192.168.240.1 port 49400 ssh2
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | |2026 年 9 月 17 日 08:54:08，时区东八区（+08:00）
| Where 在哪里 | |主机 `zx-VMware-Virtual-Platform`；登录来源 IP 为 192.168.240.1，来源端口 49400
| Who 谁 | |记录工具为 记录程序为 `sshd`（进程号 2927）；尝试登录的账号为 `zx`
| What 做了什么 | |发起 SSH 远程登录的密码认证校验
| Result 结果如何 | |发起 SSH 密码认证成功，关键词 `Accepted password`，SSH 登录会话建立成功

**用一两句话解释这个事件：**2026 年 9 月 17 日 08:54:08，来自 IP 192.168.240.1 的客户端向本机发起 SSH 登录请求，使用账号 zx 进行密码验证，验证通过后成功建立 SSH 连接

> 填写：2026 年 9 月 17 日 09:24:08，系统软件包管理工具 dpkg 记录了 htop 软件包已完成安装的状态，该日志未标注执行安装操作的具体用户账号
...
日志原文：2026-09-17 09:24:08 status installed htop:amd64 3.3.0-4build1

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | |2026 年 9 月 17 日 09:24:08；该日志未提供时区
| Where 在哪里 | |该日志未提供主机名；取自本人 Ubuntu 虚拟机的 `/var/log/dpkg.log`
| Who 谁 | |记录工具为 `dpkg`；该日志未提供执行操作的用户账号
| What 做了什么 | |记录 `htop` 软件包的安装状态
| Result 结果如何 | |状态为 `installed`，即已安装完成
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 内置的日志守护进程，负责收集内核、系统服务产生的日志，以二进制结构化格式存储，支持按服务、时间等多维度快速检索；rsyslog 是传统 syslog 服务，负责将日志持久化写入纯文本文件用于长期归档与文本检索。同一条日志会被两者同时保留，是因为 journald 作为日志接收中枢接收原始日志后，rsyslog 可从中同步日志内容并写入文本文件，二者功能互补，分别适配结构化查询和持久化归档的不同场景

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：`Failed password` 能证明发生了一次 SSH 登录尝试，且该次尝试因密码错误认证失败，同时可以确认尝试登录的账号与来源 IP 地址。它不能证明该次尝试是恶意攻击行为，也无法确认登录尝试者的真实身份，更不能代表对应账号已被攻破
