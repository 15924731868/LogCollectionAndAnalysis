...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.185.128____。
...
> 记录：本机主机名为 __b-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-17T08:53:21 |b | 192.168.185.128|Accepted password |journalctl / /var/log/auth.log |
| 失败认证 | 2026-09-17T08:53:08| b|192.168.185.128 |Failed password |journalctl / /var/log/auth.log |
...
| 两处记录有哪些共同字段或正文 | student_id=2024010029 name=李欧 action=write_test result=success|
| 两处输出的主要区别 |journalctl 自带时间戳、系统元信息；syslog 是传统文本日志，时间格式简化，以文本行存储 |
...
```text
获取命令：sudo journalctl -t lab3_read --since "5 minutes ago" --no-pager
日志原文：9月 17 08:55:46 b-VMware-Virtual-Platform lab3_read[3928]: student_id=2024010029 name=李欧 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17 08:55:46 |
| Where 在哪里 | 主机 `b‑VMware‑Virtual‑Platform`|
| Who 谁 |标签`lab3_read`（进程号 3928）；由用户 b 发起写入，正文标识学号 2024010029，姓名李欧|
| What 做了什么 |向本机日志系统写入一条 write_test 自定义测试消息 |
| Result 结果如何 |日志正文标记`result=success`，该字符串为手动写入的测试标记；journal 与 syslog 两处均可查询到本条记录 |

**用一两句话解释这个事件：**

> 填写：2026‑09‑17 08:55:46，主机 b‑VMware‑Virtual‑Platform 上用户 b 执行 logger 命令，写入一条携带学号、姓名的测试日志；`result=success`只是人为填写的文本标记，并非系统给出的执行成功判定。
...
日志原文：2026-09-17T08:53:08.223298+08:00 b-VMware-Virtual-Platform sshd[3286]: Failed password for b from 192.168.185.128 port 59318 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17 08:53:08 |
| Where 在哪里 |主机 `b‑VMware‑Virtual‑Platform` |
| Who 谁 |`sshd`(进程号 3286) 记录事件；来源 IP192.168.185.128 的客户端，尝试登录账号 b |
| What 做了什么 |客户端尝试使用密码进行 SSH 登录 |
| Result 结果如何 |密码认证失败，日志关键字`Failed password`|

**用一两句话解释这个事件：**

> 填写：2026‑09‑17 08:53:08，来自 192.168.185.128 的客户端尝试对账号 b 进行 SSH 密码登录，sshd 记录本次密码认证失败。仅凭本条日志无法确定密码认证失败的具体原因。
...
日志原文：2026-09-17 09:02:42 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17 09:02:42 |
| Where 在哪里 |该日志未提供主机名；记录来自本机 Ubuntu 虚拟机`/var/log/dpkg.log` |
| Who 谁 |记录工具为 dpkg；**该日志未提供执行操作的用户账号** |
| What 做了什么 | 记录 htop 软件包的软件状态|
| Result 结果如何 |软件包状态标记为`installed`，代表 htop 处于已安装状态；仅凭该行不能区分首次安装或升级 |

**用一两句话解释这个事件：**

> 填写：2026-09-17 09:02:42，软件包管理工具 dpkg 记录了 htop 软件包已安装的状态，该日志条目没有说明是哪位用户执行的操作。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：`systemd-journald`是 systemd 自带日志服务，接收内核、服务、应用输出日志，存放在 journal 数据库（二进制），使用 journalctl 读取。
`rsyslog`是传统 syslog 服务，接收 journald 转发过来的日志，保存为文本文件（/var/log/syslog、/var/log/auth.log）。
同一条日志会保存两份：journald 先接收日志；再转发给 rsyslog，rsyslog 把日志写入文本日志文件，因此两边都能查到同一条事件。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：可以证明：IP 为 192.168.185.128 的客户端，在对应时间尝试使用账号 b 进行 SSH 密码登录，本次密码认证失败。
不能证明：不能断定操作者身份；不能断定是攻击者暴力破解，也不能断定是合法用户输错密码；单凭该日志无法得知密码失败的具体原因。
