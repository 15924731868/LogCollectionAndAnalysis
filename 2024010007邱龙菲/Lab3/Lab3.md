...
> 记录：本次使用的 Ubuntu 用户名为 ____qlf__。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.43.128___。
...
> 记录：本机主机名为 ____qlf-VMware-Virtual-Platform__（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |Sep 22 17:36:28|qlf |192.168.43.1 | Accepted password|journal（journalctl -u ssh 输出） |
| 失败认证 |Sep 22 17:36:21|qlf | 192.168.43.1|Failed password | journal（journalctl -u ssh 输出）|
...
| 两处记录有哪些共同字段或正文 |`student_id=2024010007 name=邱龙菲 action=write_test result=success`，全部正文内容一致|
| 两处输出的主要区别 |journalctl 输出携带完整时间时区`2026‑09‑22T17:53:25.430475+08:00`；syslog 文本日志是传统 syslog 格式，时间格式、排版字段与 journal 不同 |
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026‑09‑22 17:53:23，日志包含时区 + 08:00|
| Where 在哪里 |主机 `qlf‑VMware‑Virtual‑Platform` |
| Who 谁 |日志标签`lab3_read`，进程号 4056；正文标识学号 2024010007，姓名邱龙菲|
| What 做了什么 | 使用 logger 工具向本机日志系统写入一条结构化测试日志消息|
| Result 结果如何 |正文标记`result=success`，该字符串为人为写入测试标记；journal 与 syslog 均成功查询到这条记录 |

**用一两句话解释这个事件：**

> 填写：2026‑09‑22 17:53:23，在 qlf‑VMware‑Virtual‑Platform 主机上执行 logger 命令生成测试日志，携带学号、姓名结构化字段；日志中的 success 只是写入文本标记，不是系统自动判定执行结果。
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9 月 22 日 17:34:29 |
| Where 在哪里 |主机：qlf‑VMware‑Virtual‑Platform |
| Who 谁 |记录程序 sshd；尝试登录账号 qlf；来源 IP：192.168.43.1|
| What 做了什么 | 外部客户端向本机 SSH 服务发起密码登录，提交 qlf 账号密码完成身份认证|
| Result 结果如何 |`Accepted password`密码认证通过，成功建立 SSH 会话 |

**用一两句话解释这个事件：**

> 填写：IP 地址 192.168.43.1 的客户端使用 qlf 账号密码登录 Ubuntu 虚拟机，密码正确，SSH 登录成功，开启远程会话。
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026‑09‑22 18:04:44|
| Where 在哪里 |主机：qlf‑VMware‑Virtual‑Platform |
| Who 谁 |dpkg 包管理工具，处理`htop:amd64`软件包|
| What 做了什么 |dpkg 完成 htop 软件包解压、配置，执行软件安装流程 |
| Result 结果如何 |status installed，htop‑3.3.0‑4build1 版本安装完成，状态为已安装 |

**用一两句话解释这个事件：**

> 填写：2026‑09‑22 18:04:44，本机 dpkg 工具完成 htop 软件包的全部安装流程，软件状态标记为 installed，htop 工具成功部署到系统。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：`systemd‑journald`是 systemd 自带日志服务，接收系统、程序输出，以二进制格式存储日志，通过`journalctl`读取。`rsyslog`是传统日志服务，接收日志消息，输出保存为`/var/log`下的文本日志文件。logger 产生的日志先交给 journald，journald 一方面存入自身二进制日志，同时转发这条消息给 rsyslog 写入 syslog 文本文件，因此同一条日志会在两处同时保存。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：可以证明来自 192.168.43.1 的主机向本机 ssh 服务使用 qlf 账号发起登录，密码校验没有通过，登录被拒绝。但不能证明操作者身份，无法确定是本人输错密码还是外部人员暴力尝试，单凭该日志不能断定就是黑客攻击。
