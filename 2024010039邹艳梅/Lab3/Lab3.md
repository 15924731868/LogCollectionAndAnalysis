...
> 记录：本次使用的 Ubuntu 用户名为 __zym____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.30.128____。
...
> 记录：本机主机名为 ___zym-VMware-Virtual-Platform___（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |9月 17 09:05:49 zym-VMware-Virtual-Platform sshd[10680]: Accepted password for zym from 192.168.30.1 port 56171 ssh2 |zym |192.168.30.1 | Accepted password| journal|
| 失败认证 |9月 17 09:05:44 zym-VMware-Virtual-Platform sshd[10680]: Failed password for zym from 192.168.30.1 port 56171 ssh2 | zym| 192.168.30.1| Failed password|journal |
...
| 项目 | 你的记录 |
| :--- | :--- |
| journal 中是否查到 |是，9月 17 09:22:14 zym-VMware-Virtual-Platform lab3_read[10906]: student_id=2024010039 name=zym action=write_test result=success |
| `/var/log/syslog` 中是否查到 |是，2026-09-17T09:22:14.535879+08:00 zym-VMware-Virtual-Platform lab3_read: student_id=2024010039 name=zym action=write_test result=success |
| 两处记录有哪些共同字段或正文 |主机名：`zym-VMware-Virtual-Platform`；标签`lab3_read`；内容完全一致：`student_id=2024010039 name=zym action=write_test result=success` |
| 两处输出的主要区别 | 1. 时间格式：journal 为`9月 17 09:22:14`；syslog 是带微秒与时区的 ISO 格式`2026-09-17T09:22:14.535879+08:00`2. 进程 PID：journal 输出带有进程号`[10906]`，syslog 不输出 PID3. 存储形式：journal 存于二进制数据库；syslog 是纯文本文件|
...
```text
获取命令：logger -p user.notice -t lab3_read 'student_id=2024010039 name=zym action=write_test result=success'

日志原文：9月 17 09:22:14 zym-VMware-Virtual-Platform lab3_read[10906]: student_id=2024010039 name=zym action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9 月 17 日 09:22:14 |
| Where 在哪里 | zym-VMware-Virtual-Platform|
| Who 谁 | 标签`lab3_read`（进程号 10906）写入；正文标识本人 zym，学号 2024010039|
| What 做了什么 | - 使用 logger 命令向本机日志系统写入一条 write_test 测试日志消息|
| Result 结果如何 |- 正文标记`result=success`；journal 和 syslog 两处日志都可以查询到该记录，验证写入成功 |

**用一两句话解释这个事件：**

> 填写：9 月 17 日 09:22:14，我在 zym-VMware-Virtual-Platform 虚拟机主机上使用 logger 命令，写入一条带学号的结构化测试日志，日志标记结果为 success，在 journal 与 syslog 中均可检索到。
...
日志原文：`2026-09-17T09:05:49.561116+08:00 zym-VMware-Virtual-Platform sshd[10680]: Accepted password for zym from 192.168.30.1 port 56171 ssh2`
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17 09:05:49 |
| Where 在哪里 |本机主机`zym-VMware-Virtual-Platform`；登录来源 IP：`192.168.30.1`，端口 56171 |
| Who 谁 | 由程序`sshd`记录；尝试登录账号为`zym`|
| What 做了什么 |客户端通过 SSH 协议，使用密码方式尝试登录这台虚拟机 |
| Result 结果如何 |`Accepted password`，密码认证成功，SSH 登录通过 |

**用一两句话解释这个事件：**

> 填写：2026 年 09 月 17 日 09:05:49，来自 192.168.30.1 的客户端使用账号 zym 通过 SSH 密码登录虚拟机，sshd 验证密码正确，SSH 登录认证成功。
...
```text
实际日志来源（使用替代来源时说明原因）：本机 Ubuntu 虚拟机 `/var/log/dpkg.log`
获取命令：grep "htop" /var/log/dpkg.log | tail -n 10
日志原文：`2026-09-17 10:27:02 status installed htop:amd64 3.3.0-4build1`
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 17 日 10:27:02 |
| Where 在哪里 | 本机虚拟机，日志取自 `/var/log/dpkg.log`|
| Who 谁 |记录工具为 dpkg；日志未记录执行操作的用户账号 |
| What 做了什么 |记录 htop 软件包的安装状态 |
| Result 结果如何 |状态为`installed`，htop 软件包安装完成 |

**用一两句话解释这个事件：**

> 填写：2026 年 09 月 17 日 10:27:02，dpkg 工具记录 htop 软件包状态为已安装，该日志不记录是哪个用户执行的安装命令。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 收集系统内核、服务产生的日志，以二进制形式存储；rsyslog 接收日志，输出成文本日志文件。journald 捕获日志消息后会转发一份给 rsyslog，因此同一条事件会被两者同时保存。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：能证明有客户端使用该账号发起 SSH 登录，密码校验失败。不能证明这次登录操作就是该账号本人所为，仅来源 IP 只能代表连接地址，无法确认实际操作者身份。
