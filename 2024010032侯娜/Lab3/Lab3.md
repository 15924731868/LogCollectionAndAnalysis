...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __**192.168.250.128**__。
...
> 记录：本机主机名为 _**hn-VMware-Virtual-Platform**_（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-17T08:16:01|hn|192.168.250.1|Accepted password|/var/log/auth.log|
| 失败认证 |2026-09-17T08:15:54|hn|192.168.250.1|Failed password|/var/log/auth.log|
...
| 项目 | 你的记录 |
| :--- | :--- |
| journal 中是否查到 |是，可以查到本次 logger 写入的日志记录|
| `/var/log/syslog` 中是否查到 |是，可以查到同一条 logger 写入的日志记录|
| 两处记录有哪些共同字段或正文 |学号`student_id=2024010032`、姓名`name=侯娜`、`action=write_test`、`result=success`、主机名、日志时间、日志标签 lab3_read，消息正文完全一致|
| 两处输出的主要区别 |journalctl 输出格式简洁，直接显示本地时间`Sep 17 08:34:48`，附带进程 PID；syslog 日志带带时区的 ISO 完整时间戳`2026-09-17T08:35:10.029185+08:00`，无进程 PID 字段。底层存储方式不同：journal 是 systemd 二进制日志；syslog 是纯文本文件。|
...
```text
获取命令：sudo grep -a -F "student_id=2024010032" /var/log/syslog | tail -n 5
日志原文：2026-09-17T08:35:10.029185+08:00 hn-VMware-Virtual-Platform lab3_read: student_id=2024010032 name=侯娜 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 17 日 08:35:10；带 + 08:00 东八区时区、包含毫秒 |
| Where 在哪里 |主机 hn-VMware-Virtual-Platform，日志文件 /var/log/syslog |
| Who 谁 |自己指定的标签 lab3_read；正文中标识本人姓名侯娜，学号 2024010032 |
| What 做了什么 |向本机 syslog 日志系统写入一条 write_test 自定义测试消息 |
| Result 结果如何 |正文标记 result=success，日志成功写入 syslog，可以查询到记录 |

**用一两句话解释这个事件：**

> 填写：
**2026 年 09 月 17 日 08:35:10，在 hn-VMware-Virtual-Platform 虚拟机上，使用 logger 工具写入一条带本人学号姓名的测试日志，标记本次 write_test 操作成功，日志持久保存到 syslog。**
...
日志原文：2026-09-17T08:30:31.04051793+08:00 hn-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session opened for user root(uid=0) by hn(uid=1000)
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 17 日 08:30:31；带 + 08:00 东八区时区 |
| Where 在哪里 |主机 hn-VMware-Virtual-Platform，日志文件 /var/log/auth.log |
| Who 谁 |认证程序 pam_unix，用户 hn (uid=1000)，申请切换至 root 用户 |
| What 做了什么 |hn 用户执行 sudo 命令，进行身份校验，打开 root 会话 |
| Result 结果如何 |sudo 身份认证成功，root 会话成功打开 |

**用一两句话解释这个事件：**

> 填写：
**2026-09-17 08:30:31，虚拟机用户 hn 执行 sudo，pam_unix 完成身份校验，成功打开 root 会话，记录在 auth.log。**
...
日志原文：2026-09-17 08:31:32 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 17 日 08:31:32；该日志未提供时区 |
| Where 在哪里 |该日志未提供主机名；从本人 Ubuntu 虚拟机的 /var/log/dpkg.log 获取 |
| Who 谁 |记录工具为 dpkg；日志未提供执行操作的用户账号 |
| What 做了什么 |记录 htop 软件包的安装状态 |
| Result 结果如何 |状态为 installed，htop 软件包已经成功安装 |

**用一两句话解释这个事件：**

> 填写：
**9 月 17 日 08:31:32，dpkg 软件包管理器记录 htop 软件包状态为 installed，代表 htop 已经完成安装，日志不记录操作人。**
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：
   **systemd-journald 是 systemd 自带日志服务，收集系统全部日志并保存为二进制日志；rsyslog 是传统 syslog 服务，接收日志并保存为文本日志到`/var/log`。journald 收到日志后，会转发一份副本给 rsyslog，二者各自存储，所以能同时保留同一条日志。**

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：
   **`Failed password`可以证明：有来自对应 IP 的客户端尝试 SSH 登录，目标账号存在，且提交的密码校验失败。
不能证明：不能确定攻击者知道这个账号，也无法确认本次尝试是人为手动操作还是自动化脚本爆破。**
