...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 <u>192.168.79.128</u>。
...
> 记录：本机主机名为 <u>wfe-VMware-Virtual-Platform</u>（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 | 2026-09-17 08:15:50| wfe|192.168.79.1 |Accepted password |/var/log/auth.log、journalctl系统日志 |
| 失败认证 |2026-09-17 08:15:39 |wfe | 192.168.79.1| Failed password|/var/log/auth.log、journalctl系统日志  |
...
| 两处记录有哪些共同字段或正文 |核心正文完全一致： student_id=2024010034 name=王凤娥 action=write_test result=success ；均包含日志时间、主机名、进程标识、日志标签  lab3_read  等共同字段 |
| 两处输出的主要区别 |时间格式：journalctl为本地简易时间；syslog是带毫秒与时区的ISO格式。           字段：journalctl显示进程号，syslog不显示。 输出类型：journalctl是结构化日志；syslog是原始文本日志 |
...
```text
获取命令：logger -p user.notice -t lab3_read "student_id=2024010034 name=王凤娥 action=write_test result=success"
日志原文：student_id=2024010034 name=王凤娥 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月17日 08:21:19 |
| Where 在哪里 | 主机 wfe-VMware-Virtual-Platform（Ubuntu虚拟机） |
| Who 谁 |自己指定的标签 lab3_read（进程号4265）写入；正文标识本人姓名为王凤娥，学号2024010034  |
| What 做了什么 | 向本机日志系统写入一条write_test测试消息|
| Result 结果如何 | 正文写明 result=success，这条日志可通过journalctl与syslog两处查询命令检索到|

**用一两句话解释这个事件：**

> 填写：9月17日08:21:19，本人在Ubuntu虚拟机上使用logger命令，写入一条带学号、姓名的测试日志，日志写入成功，可在两套日志系统中查到记录。
...
```text
获取命令：sudo grep -E "Accepted|Failed password|sudo" /var/log/auth.log
日志原文：2026-09-17T08:15:50.664250+08:00 wfe-VMware-Virtual-Platform sshd[3871]: Accepted password for wfe from 192.168.79.1 port 60201 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-17 08:15:50，带+08:00时区 |
| Where 在哪里 |  主机 wfe-VMware-Virtual-Platform（Ubuntu虚拟机），日志文件 /var/log/auth.log  |
| Who 谁 | 记录程序：sshd；尝试登录账号：wfe；来源IP：192.168.79.1 |
| What 做了什么 | 客户端通过SSH协议，使用密码方式尝试登录虚拟机wfe账号|
| Result 结果如何 | Accepted password，SSH密码认证成功，登录会话建立|

**用一两句话解释这个事件：**

> 填写：2026-09-17 08:15:50，IP地址192.168.79.1的客户端发起SSH登录，向Ubuntu虚拟机的wfe账号提交密码，密码校验通过，SSH认证成功。
...
| Where 在哪里 |  该日志未提供主机名`/var/log/dpkg.log` 取得” |
...
日志原文：2026-09-17 08:29:57 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026年09月17日 08:29:57|
| Where 在哪里 |主机 wfe-VMware-Virtual-Platform（Ubuntu虚拟机）|
| Who 谁 | 账号：wfe ;记录工具为 dpkg；|
| What 做了什么 | 记录 htop 软件包的状态|
| Result 结果如何 | 状态为 installed，即已安装；架构amd64，版本3.3.0-4build1|

**用一两句话解释这个事件：**

> 填写：9月17日08:29:57，软件包管理工具dpkg记录了htop软件包已安装的状态，这条日志没有说明是哪个用户执行的安装操作。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写： systemd-journald 是systemd配套日志服务，收集内核、服务、程序日志，保存在journal二进制日志； rsyslog 是传统syslog服务，接收日志并输出为文本格式日志文件。日志会先发给journald，journald转发副本给rsyslog，两个服务各自存储副本，因此可以同时保留同一条日志。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：SSH提示 Failed password ，能证明：已经成功建立网络连接、用户名存在，只是密码校验失败。不能证明：不能确定攻击者是真实用户，也不能证明该IP一定是攻击源（可伪造），仅说明本次密码尝试错误。
