...
> 记录：本次使用的 Ubuntu 用户名为 __kui____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.66.128____。
...
> 记录：本机主机名为 __kui-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 | 2026-09-22 21:36:01| kui| 192.168.66.1| Accepted password| /var/log/auth.log|
| 失败认证 | 2026-09-22 21:35:57| kui| 192.168.66.1| Failed password| /var/log/auth.log|
...
```text
获取命令：logger -p user.notice -t lab3_read "student_id=2024010002 name=王诗惠 action=write_test result=success"
日志原文：student_id=2024010002 name=王诗惠 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-22 21:41:28|
| Where 在哪里 | kui-VMware-Virtual-Platform|
| Who 谁 | kui|
| What 做了什么 | 使用 logger 命令，以 lab3_read 标签写入一条自定义测试系统日志|
| Result 结果如何 | 日志成功写入，journald 和 syslog 均可查询到该记录

**用一两句话解释这个事件：**

> 填写：用户 kui 执行 logger 指令，向系统写入包含学号、姓名的自定义测试日志，日志成功保存。
...
日志原文：2026-09-22 21:58:25 install htop:amd64 <none> 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-22 21:58:25|
| Where 在哪里 | kui-VMware-Virtual-Platform 虚拟机|
| Who 谁 | kui|
| What 做了什么 | 执行`sudo apt install -y htop`，系统安装 htop 软件包，dpkg 记录软件安装事件|
| Result 结果如何 | htop 软件包成功安装到系统，dpkg 日志记录完整安装流程|

**用一两句话解释这个事件：**

> 填写：用户 kui 通过 apt 命令安装 htop 工具，dpkg 记录下本次软件安装事件，标记 htop 成功安装。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 自带日志服务，收集内核、服务、程序日志，保存在二进制 journal 文件；rsyslog 是传统 syslog 服务，接收日志并保存为文本格式（如 syslog、auth.log）。日志产生后会同时发送给 journald 和 rsyslog，因此同一条日志会在两套系统中分别存储，实现同时保留。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password能证明有客户端发起 SSH 登录尝试，且本次密码校验失败。不能证明攻击者账号存在、IP 真实，也不能证明登录行为最终成功，仅代表单次密码输入错误。
