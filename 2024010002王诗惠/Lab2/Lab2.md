...
> 记录：Ubuntu 的日期和时间为 __ 四 2026-09-10 09:11:57____；时区为 __Asia/Shanghai____；时间同步状态为 __yes____。
...
> 记录：`/var/log/syslog` 是否存在？__是____；实际输出：__syslog exists
____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.66.128 
____。
...
> 记录：22 端口是否正在监听？__是____；输出中的本地地址和端口为 __ 0.0.0.0:22 __[::]:22 __。
...
> 记录：SSH 登录后的主机名为 __kui-VMware-Virtual-Platform
____。
...
> 记录：当前目录为 __/home/kui
____；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：选取的一条日志来自哪个程序或服务？__sudo____；记录了什么事件？__用户 kui 在 pts/0 终端，使用 sudo 提升至 root 权限，执行 journalctl -n 30 --no-pager 命令查询系统日志____。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 __kernel: [Firmware Bug]: TSC doesn't count with P0 frequency!（固件 BUG，TSC 时间计数器无法使用 P0 频率计数）____。
...
> 填写：/var/log/wtmp与/var/log/btmp是二进制文件，cat 只能读取文本，直接查看会出现乱码。查看 wtmp 用last命令，查看 btmp 用lastb命令。
