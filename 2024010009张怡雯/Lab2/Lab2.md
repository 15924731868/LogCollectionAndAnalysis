...
> 记录：Ubuntu 的日期和时间为 _ 2026-09-17 15:54:02 ___；时区为 __Asia/Shanghai (CST, +0800)____；时间同步状态为 __yes____。
...
> 记录：`/var/log/auth.log` 是否存在？__是____；实际输出：_auth.log exists_____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.134.129____。
...
> 记录：22 端口是否正在监听？_是_____；输出中的本地地址和端口为 __0.0.0.0:22        0.0.0.0:*____。
...
> 记录：SSH 登录后的主机名为 _zyw-VMware-Virtual-Platform_____。
...
> 记录：当前目录为 __/home/zyw____；是否确认已通过 Git Bash 登录 Ubuntu？___是___。
...
> 记录：`/var/log/journal/` 的检查结果为 _root systemd-journal 4096  9月  3 08:19_____；`/run/log/journal/` 的检查结果为 __ root systemd-journal   40  9月 17 15:48____。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog |系统全局日志 |文本 |sudo tail -n 20 /var/log/syslog |
...
> 记录：选取的一条日志来自哪个程序或服务？_sudo_____；记录了什么事件？_用户zyw执行了journalctl命令来查看最近的30条日志_____。
...
> 记录：实际查询起点为 _2026-09-17 16：00：00_____；终点为 __2026-09-17 18：18：00____；观察到的事件或无记录情况为 _观察到今日的登录、系统服务等日志_____。
...
> 记录：是否查到匹配日志？_是_____；其中一条的内容或无记录提示为 _9月 17 18:12:51 zyw-VMware-Virtual-Platform gnome-shell[2604]: GFileInfo created without standard::icon
_____。
...
> 填写：因为/var/log/wtmp和/var/log/btmp是二进制文件。直接用cat读取会出现乱码，无法阅读
