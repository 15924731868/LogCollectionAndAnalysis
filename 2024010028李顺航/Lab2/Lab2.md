Lab2：Linux 日志认识与查询
本实验承接 Lab1 操作手册 和 Lab1 验收作业。开始前应已完成 VMware、Ubuntu、网络、虚拟硬件、VMware Tools、SSH 和 rsyslog 的安装与验收。
...
以下命令在 Ubuntu 终端，或已通过 SSH 登录 Ubuntu 的窗口中逐条执行。先检查工具是否可用：
...
表格左列是存储位置，右列是读取工具。例如 /var/log/wtmp 是文件，last 是读取它的命令；journalctl -k 读取 journal 中的内核消息，本身不是另一个日志文件。
...
记录：journald 的实际运行状态为 __active____。
...
记录：rsyslog 的实际运行状态为 __active____。
...
记录：Ubuntu 的日期和时间为 __2026‑09‑16 22:28____；时区为 __Asia/Shanghai____；时间同步状态为 __system clock synchronized: yes____。
...
记录：/var/log/syslog 是否存在？存在；实际输出：syslog exists。
...
记录：/var/log/auth.log 是否存在？存在；实际输出：auth.log exists。

如果服务未运行、时间错误，或文件检查没有预期输出，先按 Lab1 操作手册修复。syslog 和 auth.log 是本实验的必需环境，检查通过后继续 3.3 节。
...
需要了解软件时，可查看官方网站：Xshell、Xftp、MobaXterm。本段供了解，本次操作和截图仍按下面的 Git Bash 步骤完成。

在 Ubuntu 中取得连接信息
以下四条命令都在 Ubuntu 虚拟机终端中执行，一条一条输入并查看输出。
...
记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.108.128____。
...
记录：22 端口是否正在监听？__是____；输出中的本地地址和端口为 __0.0.0.0:22、[::]:22____。
...
记录：SSH 登录后的主机名为__lsh-VMware-Virtual-Platform____。
...
记录：当前目录为/home/lsh ；是否确认已通过 Git Bash 登录 Ubuntu？是。
...
记录：/var/log/journal/ 的检查结果为 __目录，存在____；/run/log/journal/ 的检查结果为 __符号链接，指向 /var/log/journal____。

在此填写日志盘点表：根据刚才的目录和类型检查，登记 5 个真实存在的日志文件或目录 即可。不存在的路径不计入这 5 项；命令名也不能当作文件路径。可从 3.1 节查阅用途和读取工具，也可选用其他真实日志。

实际路径	主要用途	文本、二进制还是目录	使用什么命令
/var/log/syslog		系统综合 syslog 消息，记录各类系统服务事件	文本	sudo tail、sudo grep
/var/log/auth.log		SSH、sudo、登录认证授权事件		文本	sudo tail、sudo grep
/var/log/wtmp		成功登录、注销、重启历史记录		二进制		last
/var/log/btmp		登录失败历史记录	二进制	sudo lastb
/var/log/journal		systemd‑journald 二进制日志存储位置		目录	journalctl
先读取文本日志
...
记录：选取的一条日志来自哪个程序或服务？NetworkManager；记录了什么事件？处理网络设备状态变更。
...
记录：实际查询起点为 __2026‑09‑16 22:36:00____；终点为 __2026‑09‑16 22:40:00____；观察到的事件或无记录情况为 __输出该时间段系统服务运行相关日志____。
...
记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 __内核警告，硬件相关提示信息____。

保存 imgs/lab2_journal_queries.png，只需覆盖以上 3 类查询的命令和关键输出。可合理拼图，长输出保留代表性记录，无匹配记录时保留真实提示。
...
答：`/var/log/wtmp` 和 `/var/log/btmp`属于二进制数据文件，cat 直接读取只会输出乱码，无法解析可读信息。读取`/var/log/wtmp`使用`last`命令；读取`/var/log/btmp`使用`sudo lastb`命令。
...
六、截图与提交要求
本次单独提交 1 份 Markdown 报告和 3 张截图。图片引用已放在各操作后面，下表用于提交前核对。

操作位置	截图必须体现的内容	文件名
3.3 节	Git Bash 中的 SSH 连接命令，以及登录后的 whoami、hostname、pwd 输出	lab2_ssh_login.png
3.4 节	/var/log 盘点和 file 的关键输出	lab2_log_inventory.png
3.5 节	三类必做查询的命令与关键输出；无记录时保留实际提示	lab2_journal_queries.png
使用电脑截图功能，文字清晰可读，严禁手机拍摄屏幕。
截图应同时显示命令和对应输出，并能辨认本人用户名、主机名或 IP。
...
└── Lab2/
    ...
    └── imgs/
        ├── lab2_ssh_login.png
        ├── lab2_log_inventory.png
        └── lab2_journal_queries.png
按 3.6 节核对本次完成情况，再打开 Lab2.md 检查三张图片能否显示。选读内容可以保留或删除，无需填写。