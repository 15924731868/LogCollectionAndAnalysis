...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 192.168.241.128 
。
...
> 记录：本机主机名为 wxt-VMware-Virtual-Platform
（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |9月17日 08:50:59 |wxt |192.168.241.1 |Accepted password |journalctl -u ssh
| 失败认证 |9月17日 08:50:53 |wxt |192.168.241.1 |Failed password |journalctl -u ssh |
...
| 两处记录有哪些共同字段或正文 |student_id=2024010004、name=wxt、action=write_test、result=success、标签lab3_read、时间 |
| 两处输出的主要区别 |journalctl会附带systemd的元数据（进程ID等）；syslog是传统格式，信息更精简，没有journal独有的元数据 |
...
```text
获取命令：sudo grep -a "student_id=2024010004" /var/log/syslog | tail -n 5 
日志原文：Sep 17 09:10:57 wxt-VMware-Virtual-Platform lab3_read[4301]: student_id=2024010004 name=wxt action=write_test result=success 
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月17日 09:10:57 |
| Where 在哪里 |主机 wxt‑VMware‑Virtual‑Platform |
| Who 谁 |标签 lab3_read（进程号4301）写入；学号2024010004，姓名wxt |
| What 做了什么 |向本机日志系统写入一条 write_test 自定义测试消息 |
| Result 结果如何 |正文标记 result=success，为自行写入的标记，非系统判定结果 |

**用一两句话解释这个事件：**

> 填写：9月17日09:10:57，我在wxt‑VMware‑Virtual‑Platform主机上，使用logger命令写入一条学号2024010004、姓名wxt的测试日志，标记结果为success。
...
日志原文：Sep 17 08:50:59 wxt‑VMware‑Virtual‑Platform sshd[3460]: Accepted password for wxt from 192.168.241.1 port 64783 ssh2 
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月17日 08:50:59 |
| Where 在哪里 |主机 wxt‑VMware‑Virtual‑Platform，来源IP：192.168.241.1 |
| Who 谁 |记录程序是sshd；尝试登录账号：wxt |
| What 做了什么 |从192.168.241.1发起SSH密码登录请求 |
| Result 结果如何 |Accepted password，SSH密码认证成功
 |

**用一两句话解释这个事件：**

> 填写：9月17日08:50:59，账号wxt从192.168.241.1向本机发起SSH登录，密码验证通过，认证成功。
...
日志原文：2026-09-17 09:32:04 status installed htop:amd64 3.3.0-4build1 

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026年9月17日 09:32:04；该日志未提供时区 |
| Where 在哪里 |从本人Ubuntu虚拟机的 /var/log/dpkg.log 取得 |
| Who 谁 |记录工具为 dpkg；日志没有显示执行操作的用户账号 |
| What 做了什么 |记录 htop 软件包的安装状态 |
| Result 结果如何 |状态为 installed，htop 软件包已完成安装 |

**用一两句话解释这个事件：**

> 填写：9月17日09:32:04，软件包管理工具dpkg记录了htop已经成功安装，日志没有记录是哪个用户执行的操作。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd‑journald 是系统自带的日志服务，收集所有程序产生的日志，存放在二进制日志中；rsyslog 负责把日志转存为文本格式，保存到  /var/log  目录下。journald先接收日志，再转发一份给rsyslog写入文件，因此同一条日志会同时在两处保存。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：能证明：有来自对应IP的主机尝试登录，并且输入的密码是错误的。不能证明：这个IP就是攻击者真实地址，也不能证明账号确实存在，更不能确定操作人是谁，IP可以伪造。
