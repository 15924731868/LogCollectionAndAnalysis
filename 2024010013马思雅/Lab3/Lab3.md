...
> 记录：本次使用的 Ubuntu 用户名为 ___msy___。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 ___192.168.136.128___。
...
> 记录：本机主机名为 ___msy-VMware-Virtual-Platform___（即 4W1R 中 Where 的取值）。
...
| 项目 | 你的记录 |
| :--- | :--- |
| journal 中是否查到 |可以查到，lab3_read标签，日志内容：student_id=2024010013 name=msy action=write_test result=success |
| `/var/log/syslog` 中是否查到 |可以查到，包含完全一样的日志正文	 |
| 两处记录有哪些共同字段或正文 |lab3_read、student_id=2024010013、name=msy、action=write_test、result=success，日志消息主体内容完全一致 |
| 两处输出的主要区别 |	journalctl输出自带进程PID、系统时间、服务元信息；syslog文件输出格式更简洁，只保留基础时间、标签与日志文本，没有进程PID等额外系统字段	 |
...
```text
获取命令：sudo grep -F "student_id=2024010013" /var/log/syslog | tail -n 5
日志原文：Sep 17 09:05:54 msy-VMware-Virtual-Platform lab3_read[4258]: student_id=2024010013 name=msy action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17 09:05:54 |
| Where 在哪里 |msy-VMware-Virtual-Platform（Ubuntu虚拟机） |
| Who 谁 |lab3_read	 |
...
> 填写：使用logger命令生成一条自定义测试日志，标签为lab3_read，成功写入系统syslog文件，可通过grep检索到这条记录。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是systemd的日志服务，收集系统所有日志，存为二进制日志；rsyslog是传统syslog服务，接收日志并写入文本文件（如/var/log/syslog）。日志产生后会同时发给两者，因此同一条日志会被两套系统分别保存。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password可以证明来自192.168.136.1的客户端尝试使用student账号进行SSH登录，密码校验失败。但不能证明操作人真实身份，也无法确认客户端主机上是谁发起的这次登录尝试。
