...
> 记录：本次使用的 Ubuntu 用户名为 litong_____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP为 192.168.222.128______。
...
> 记录：本机主机名为litong-VMware-Virtual-Platform______（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-17 08:32:21 |litong | 192.168.222.1| Accepted password| /var/log/auth.log（journalctl ssh 日志）|
| 失败认证 |2026-09-17 08:32:15 | litong|192.168.222.1 |Failed password | /var/log/auth.log（journalctl ssh 日志）|
...
| 项目 | 你的记录 |
| :--- | :--- |
| journal 中是否查到 |是，成功查到这条自定义日志 |
| `/var/log/syslog` 中是否查到 |是，成功查到这条自定义日志 |
| 两处记录有哪些共同字段或正文 |时间、标签lab3_read、日志正文完全一样：student_id=2024010027 name=李彤 action=write_test result=success |
| 两处输出的主要区别 |journalctl 时间格式简化（月日 时分秒）；syslog 是 ISO 完整时间戳，带微秒与时区。journal 是二进制日志库，syslog 是文本日志文件。 |
...
```text
获取命令：sudo grep -F "student_id=2024010027" /var/log/syslog | tail -n 5
日志原文：2026-09-17T08:49:55.154889+08:00 litong-VMware-Virtual-Platform lab3_read: student_id=2024010027 name=李彤 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9 月 17 日 08:49:55；日志包含年份与时区 |
| Where 在哪里 | 主机 litong-VMware-Virtual-Platform|
| Who 谁 |标签lab3_read写入；正文标识本人姓名李彤，学号 2024010027 |
| What 做了什么 |向本机日志系统写入一条 write_test 测试消息 |
| Result 结果如何 |正文标记result=success，可通过 journalctl 和 syslog 两处查询验证|

**用一两句话解释这个事件：**

> 填写：9 月 17 日 08:49:55，我在虚拟机主机上使用 logger 命令写入一条带学号姓名的测试日志，日志内标记本次测试动作结果为 success。
...
日志原文：Sep 17 08:32:15 litong-VMware-Virtual-Platform sshd[3560]: Failed password for litong from 192.168.222.1 port 42422 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9 月 17 日 08:32:15；该显示未提供年份和时区 |
| Where 在哪里 | 主机 litong-VMware-Virtual-Platform|
| Who 谁 |日志记录程序 sshd；尝试登录账号 litong，来源IP：192.168.222.1 |
| What 做了什么 |用户尝试通过 SSH 协议登录本机账号 litong，提交密码进行身份认证 |
| Result 结果如何| 密码认证失败，本次 SSH 登录尝试未通过 |

**用一两句话解释这个事件：**

> 填写：9 月 17 日 08:32:15，从内网 IP 发起 SSH 登录尝试，向账号 litong 提交密码，密码校验失败，登录被拒绝
...
获取命令：grep -a "htop" /var/log/dpkg.log | tail -n 10
日志原文：2026-09-17 09:08:18 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 09:08:18；该日志未提供时区 |
| Where 在哪里 |该日志未提供主机名；取自 Ubuntu 虚拟机/var/log/dpkg.log |
| Who 谁 |记录工具为 dpkg；该日志未提供执行操作的用户账号 |
| What 做了什么 |	记录 htop 软件包的安装状态 |
| Result 结果如何 |状态为 installed，htop 软件包已完成安装；仅凭本行无法区分首次安装或升级 |

**用一两句话解释这个事件：**

> 填写：9 月 17 日 09:08:18，dpkg 软件包管理器记录 htop 软件包状态为已安装，日志没有记录本次安装操作对应的用户账号。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 的日志服务，收集系统全部日志，存储为二进制日志；rsyslog 是传统 syslog 服务，接收日志并写入文本文件/var/log/syslog。logger 产生的日志会同时发送给 journald 与 rsyslog，所以两个系统可以同时保存同一条日志，也就是 3.2 节里 journalctl 和 grep syslog 都查到同一条记录。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password 能证明：有主机尝试 SSH 登录该账号，提交的密码与正确密码不匹配，本次认证失败。不能证明：不能证明登录操作是账号本人执行，也不能确认发起登录的 IP 对应的真实操作者身份。
