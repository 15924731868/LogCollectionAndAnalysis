...
| Where 在哪里 | 主机 `ubuntu` |
| Who 谁 | 自定义标签 lab3_read（进程号 6136）写入；正文中标识本人姓名为陶正洪，学号 2024010017 |
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 14:12:05；时区为 +00:00 |
| Where 在哪里 |主机 ubuntu |
| Who 谁 |SSH 服务程序 sshd（进程号 5853）记录；尝试登录的账号为 ubuntu，来源 IP 为 192.168.62.1 |
| What 做了什么 |发起一次 SSH 密码登录尝试 |
| Result 结果如何 |密码认证失败，依据为日志关键字 Failed password |

**用一两句话解释这个事件：**

> 填写：9 月 17 日 14:55:36，本人在 ubuntu 主机上通过 logger 命令写入了一条带本人学号姓名的测试日志，日志正文手动标记了 write_test 动作和 success 结果，该日志成功被系统日志服务收录。
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 15:01:46；该日志未提供时区 |
| Where 在哪里 |记录来自本人 Ubuntu 虚拟机的 /var/log/dpkg.log|
| Who 谁 |软件包管理工具 dpkg 记录 |
| What 做了什么 |记录 htop 软件包的安装状态变更 |
| Result 结果如何 |状态为 installed，即 htop 软件包已完成安装 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日 15:01:46，系统软件包管理工具 dpkg 记录了 htop 软件包的状态为已安装，代表本次 htop 安装操作执行完成，该条日志未体现执行操作的具体用户。
