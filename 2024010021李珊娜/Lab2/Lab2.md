...
> 记录：Ubuntu 的日期和时间为 ____2026-09-10 08:42:23（星期四）CST__；时区为 ___Asia/Shanghai (CST, +0800)___；时间同步状态为 __已同步____。
...
> 记录：`/var/log/syslog` 是否存在？____存在__；实际输出：___syslog exists___。
...
> 记录：`/var/log/auth.log` 是否存在？____存在__；实际输出：___auth.log exists___。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 ____0.0.0.0:22__。
...
> 记录：SSH 登录后的主机名为_ lsn-VMware-Virtual-Platform_。
...
> 记录：当前目录为 ___/home/lsn___；是否确认已通过 Git Bash 登录 Ubuntu？____是__。
...
> 填写：答：因为 /var/log/wtmp 和 /var/log/btmp 是二进制格式的登录数据库文件（file 命令识别为 data），不是文本文件，cat 按文本逐行读取只会输出乱码、无法读懂。读取成功登录 / 注销 / 重启历史应使用 last（读 wtmp），读取失败登录历史应使用 sudo lastb（读 btmp）。
