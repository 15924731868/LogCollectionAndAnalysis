...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 192.168.252.128 。
...
> 记录：本机主机名为 yanglian-VMware-Virtual-Platform（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-17 08:32:57|yanglian|192.168.252.1|Accepted|/var/log/auth.log|
| 失败认证 |2026-09-17 08:32:49|yanglian|192.168.252.1|Failed|/var/log/auth.log|
...
| 两处记录有哪些共同字段或正文 |核心日志正文完全一致：student_id=2024010033 name=杨连盛 action=write_test result=success；
基础元信息一致：主机名均为 yanglian-VMware-Virtual-Platform、日志标识均为 lab3_read、日志生成时间均为 9 月 17 日 08:43:13|
| 两处输出的主要区别 |1. 时间格式：journalctl 输出为本地化日期格式 9月 17 08:43:13；syslog 输出为 ISO 8601 标准格式 2026-09-17T08:43:13.410040+08:00，精度到微秒并带时区。
2. 进程信息：journalctl 输出附带进程 PID [3798]；本次 syslog 输出未显示进程 PID。
3. 存储与查询方式：journal 是 systemd 管理的二进制日志，用 journalctl 命令检索；/var/log/syslog 是传统纯文本日志文件，用文本工具 grep 检索。|
...
```text
获取命令：sudo grep -F "student_id=2024010033" /var/log/syslog | tail -n 5
日志原文：2026-09-17T08:43:13.410040+08:00 yanglian-VMware-Virtual-Platform lab3_read: student_id=2024010033 name=杨连盛 action=write_test result=success

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 08:43:13，日志为 ISO 8601 标准格式，附带东八区时区信息|
| Where 在哪里 |主机 yanglian-VMware-Virtual-Platform|
| Who 谁 |自定义日志标签 lab3_read 写入；正文标识本人姓名为杨连盛，学号 2024010033|
| What 做了什么 |向本机日志系统写入一条 write_test 测试消息|
| Result 结果如何 |正文标记 result=success，通过 grep 检索 /var/log/syslog 可命中该记录，验证日志写入成功|

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日 08:43:13，本人在 Ubuntu 主机上通过logger命令写入一条包含学号 2024010033 的测试日志，标记结果为 success，该记录可在系统 syslog 文件中成功检索到。
...
日志原文：2026-09-17T08:32:57.022025+08:00 yanglian-VMware-Virtual-Platform sshd[3568]: Accepted password for yanglian from 192.168.252.1 port 50324 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 08:32:57，日志采用 ISO 8601 标准格式，附带东八区（+08:00）时区信息|
| Where 在哪里 |主机 yanglian-VMware-Virtual-Platform|
| Who 谁 |记录程序为 sshd；尝试登录的账号为 yanglian，登录来源 IP 为 192.168.252.1|
| What 做了什么 |远端主机通过 SSH 协议向本机发起远程登录请求，进行密码身份认证|
| Result 结果如何 |密码认证成功（Accepted password），SSH 会话正常建立，账号成功登录系统|

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日 08:32:57，来自 IP 地址 192.168.252.1 的设备使用 yanglian 账号通过 SSH 协议远程登录本机，密码验证通过，成功建立 SSH 登录会话。
...
日志原文：2026-09-17 09:00:17 install htop:amd64 <none> 3.3.0-4build1
2026-09-17 09:00:17 status half-installed htop:amd64 3.3.0-4build1
2026-09-17 09:00:17 status unpacked htop:amd64 3.3.0-4build1
2026-09-17 09:00:17 configure htop:amd64 3.3.0-4build1 <none>
2026-09-17 09:00:17 status unpacked htop:amd64 3.3.0-4build1
2026-09-17 09:00:17 status half-configured htop:amd64 3.3.0-4build1
2026-09-17 09:00:17 status installed htop:amd64 3.3.0-4build1

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 09:00:17|
| Where 在哪里 |运行 Ubuntu 24.04 LTS 系统的主机 yanglian-VMware-Virtual-Platform|
| Who 谁 |系统包管理程序 dpkg；由本机管理员用户发起 htop 的安装操作|
| What 做了什么 |执行 htop 软件包的完整安装流程，依次完成半安装、解压、配置等步骤|
| Result 结果如何 |htop:amd64 3.3.0-4build1 软件包安装完成，最终状态标记为 installed|

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日 09:00:17，本机通过 dpkg 包管理器完成了 htop 系统监控工具的安装，软件版本为 3.3.0-4build1，适配 amd64 架构，安装全流程执行完毕。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 原生的日志服务，负责统一收集内核、系统服务和用户进程的日志，以二进制结构化格式存储，提供高效检索能力；rsyslog 是传统 syslog 实现，负责将日志以纯文本形式持久化到 /var/log 目录的各类日志文件中，支持日志过滤与转发。二者可以同时保留同一条日志，是因为 rsyslog 会从 systemd-journald 同步日志数据，分别以二进制和纯文本两种形式存储，因此我们写入的测试日志能同时通过 journalctl 和 /var/log/syslog 查询到。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：SSH 提示Failed password能证明有来自对应源 IP 的设备，尝试使用指定账号通过 SSH 登录本机，且本次输入的密码错误，密码认证未通过。它不能证明登录发起者的真实身份，源 IP 可能经过代理或跳板机，也无法区分是用户本人输错密码还是恶意的暴力破解尝试，不能直接判定账号存在安全泄露。
