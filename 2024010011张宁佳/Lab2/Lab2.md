...
> 记录：journald 的实际运行状态为 _ active_____。
...
> 记录：`/var/log/auth.log` 是否存在？___是___；实际输出：_auth.log exists
_____。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 ___0.0.0.0:22、[::]:22___。
...
> 记录：SSH 登录后的主机名为 ___znj-VMware-Virtual-Platform
___。
...
> 记录：当前目录为 ___/home/znj
___；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：`/var/log/journal/` 的检查结果为 /var/log/journal/：__存在__；`/run/log/journal/` 的检查结果为 _/run/log/journal/：__存在__。
...
> 记录：选取的一条日志来自哪个程序或服务？__NetworkManager____；记录了什么事件？_网卡 ens33 获取到新的 DHCP 租约，分配 IP 地址 192.168.154.129_____。
...
> 填写：wtmp 与 btmp 为二进制格式文件，cat 无法解析，输出乱码；查看 wtmp 使用 last，查看 btmp 使用 lastb。
