...
> 记录：本次使用的 Ubuntu 用户名为 __znj____。
...
> 记录：本机主机名为 __znj-VMware-Virtual-Platform
...
| 两处记录有哪些共同字段或正文 |student_id=2024010011、name=张宁佳、action=write_test、result=success；主机名、标签lab3_read、日志核心内容完全一致 |
| 两处输出的主要区别 | journalctl 输出时间格式为标准本地时间（9月22 17:32:53）；syslog 的 grep 输出是 ISO 带时区时间格式2026-09-22T17:32:53.479903+08:00；journalctl 属于 systemd-journal 二进制日志，syslog 是 rsyslog 生成的文本日志文件，存储方式不同|
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-22 17:32:53；时区 + 08:00 |
| Where 在哪里 |主机 znj-VMware-Virtual-Platform |
| Who 谁 |自己指定的标签 lab3_read；正文标识本人姓名为张宁佳，学号 2024010011 |
| What 做了什么 |向本机日志系统写入一条 write_test 测试消息 |
| Result 结果如何 |正文写明 result=success，这是自己写入的测试标记，可通过 grep 查询日志来印证 |

**用一两句话解释这个事件：**

> 填写：2026 年 09 月 22 日 17:32:53，本人在 znj-VMware-Virtual-Platform 虚拟机主机中写入一条学号 20240100111 的 write_test 测试日志，日志标记操作结果为 success。
...
日志原文：2026-09-22T16:56:17.309732+08:00 znj-VMware-Virtual-Platform sshd[8363]: Accepted password for znj from 192.168.154.1 port 59973 ssh2

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-22T16:56:17.309732+08:00|
| Where 在哪里 |来源 IP 192.168.154.1，目标主机 znj-VMware-Virtual-Platform |
| Who 谁 |尝试登录账号 znj；记录程序 sshd |
| What 做了什么 |从 192.168.154.1 的 59973 端口发起 SSH2 密码登录请求 |
| Result 结果如何 | 密码认证成功，SSH 登录通过|

**用一两句话解释这个事件：**

> 填写：2026 年 09 月 22 日 16:56:17，sshd 捕获到来自 192.168.154.1 主机的账号 znj 的 SSH 登录尝试，本次密码验证成功，建立 SSH 会话。
...
日志原文：2026-09-22 21:30:48 status installed htop:amd64 3.3.0-4build1

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |：2026-09-22 21:30:48 |
...
> 填写：2026 年 09 月 22 日 21:30:48，本机 dpkg 包管理器完成 htop 软件包的安装流程，将版本 3.3.0-4build1 的 htop 成功部署到系统中。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 自带日志服务，接收内核、服务的日志并保存为二进制 journal 日志；rsyslog 是传统 syslog 服务，接收日志并输出为文本格式日志文件。systemd-journald 会把日志转发给 rsyslog，同一条日志被两个服务分别存储，所以可以同时保留。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：SSH 的`Failed password`能证明有客户端尝试使用该账号、密码进行 SSH 登录，且密码校验失败。但**不能证明**登录请求的发起者身份，仅能拿到来源 IP，无法确定操作人是谁，也不能确认是否为真实用户操作。
