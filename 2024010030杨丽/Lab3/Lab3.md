...
> 记录：本次使用的 Ubuntu 用户名为 __yangli__。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 _192.168.161.128_。
...
> 记录：本机主机名为 __yangli-VMware-Virtual-Platform__
（即 4W1R 中 Where 的取值）。
...
| 两处记录有哪些共同字段或正文 | student_id=2024010030、name = 杨丽、action=write_test、result=success；都包含时间戳、标签 lab3_read，日志主体内容完全一致 |
| 两处输出的主要区别 | journalctl 输出是 systemd 日志，时间格式为9月 17 09:38:15；syslog 输出使用 ISO 时间格式2026-09-17T09:38:15.274538+08:00；journal 会显示进程 PID，syslog 不显示 PID |
...
```text
获取命令：sudo grep -F "student_id=2024010030" /var/log/syslog | tail -n 5
日志原文：2026-09-17T09:38:15.274538+08:00 yangli-VMware-Virtual-Platform lab3_read: student_id=2024010030 name=杨丽 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-17T09:38:15.274538+08:00；日志带有 + 08:00 时区 |
...
日志原文：2026-09-17T09:22:17.347924+08:00 yangli-VMware-Virtual-Platform sshd[3831]: Accepted password for yangli from 192.168.161.1 port 64344 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-17T09:22:17.347924+08:00 |
| Where 在哪里 | 主机：yangli-VMware-Virtual-Platform |
| Who 谁 | 记录程序 sshd；尝试登录账号：yangli，来源 IP：192.168.161.1 |
| What 做了什么 | 用户 yangli 使用密码方式，通过 SSH 协议登录本机 |
| Result 结果如何 | Accepted password，密码认证成功，SSH 登录通过 |

**用一两句话解释这个事件：**

> 填写：2026-09-17 09:22:17，在 yangli-VMware-Virtual-Platform 主机上，账号 yangli 从 192.168.161.1 发起 SSH 密码登录，sshd 校验密码正确，认证成功，该事件记录在 auth.log。
...
日志原文：2026-09-17 10:29:49 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026 年 09 月 17 日 10:29:49；该日志未提供时区 |
...
> 填写：9 月 17 日 10:29:49，软件包管理工具 dpkg 记录了 htop 软件包已安装的状态。这一行没有说明是哪位用户执行的操作。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 自带日志服务，收集内核、服务等日志，存为 journal 二进制日志；rsyslog 是传统 syslog 服务，接收日志并输出为文本格式日志文件。二者可以同时保留同一条日志，因为 journald 接收日志消息后，会转发给 rsyslog，两份日志独立存储。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：SSH 提示Failed password能证明：已经成功建立网络连接、用户名存在，但本次密码校验失败。不能证明攻击者一定知道该用户名，也不能证明攻击源 IP 真实，更不能判断是人为手动尝试还是脚本爆破。
