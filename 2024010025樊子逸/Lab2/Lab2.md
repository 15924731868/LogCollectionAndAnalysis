...
> 记录：Ubuntu 的日期和时间为 2026‑09‑13 15:10:40 CST；时区为 Asia/Shanghai；时间同步状态为 systemd‑timesyncd active。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 192.168.122.128。
...
> 记录：SSH 登录后的主机名为 hakiii‑VMware‑Virtual‑Platform。
...
> 记录：当前目录为 /home/hakiii；是否确认已通过 Git Bash 登录 Ubuntu？是。
...
> 记录：`/var/log/journal/` 的检查结果为 目录存在；`/run/log/journal/` 的检查结果为 目录存在。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog|系统综合 syslog 消息|文本|sudo tail、sudo grep|
|/var/log/auth.log|登录、sudo 等认证授权事件|文本|sudo tail、sudo grep|
|/var/log/wtmp|用户登录注销、重启历史|二进制|last|
|/var/log/btmp|	用户登录失败记录|二进制|	sudo lastb|
|/run/log/journal|systemd 临时二进制日志|目录|journalctl|
...
> 记录：选取的一条日志来自哪个程序或服务？systemd；记录了什么事件？sudo 会话打开关闭，执行 ls、tail 等管理员命令。
...
> 记录：实际查询起点为 2026‑09‑13 08:00:00；终点为 2026‑09‑13 14:00:00；观察到的事件或无记录情况为 记录 sudo 操作、系统服务启停、网络地址变更等系统事件。
...
> 记录：是否查到匹配日志？是；其中一条的内容或无记录提示为 kernel: core: CPUID marked event: 'cpu cycles' unavailable。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制数据库文件，cat 只能读取普通文本，直接 cat 会输出乱码。读取/var/log/wtmp使用last命令；读取/var/log/btmp使用sudo lastb命令。
