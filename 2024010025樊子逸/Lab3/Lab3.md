...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 192.168.122.128。
...
> 记录：本机主机名为 hakiii‑VMware‑Virtual‑Platform（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |9 月 17 08:49:21	|hakiii|192.168.122.1|Accepted password|/var/log/auth.log|
| 失败认证 |9 月 17 08:50:06|hakiii|192.168.122.1|Failed password|/var/log/auth.log|
...
| 两处记录有哪些共同字段或正文 |标签lab3_read；正文student_id=2024010025 name=樊子逸 action=write_test result=success；事件发生时间|
| 两处输出的主要区别 |journalctl 输出附带 journald 额外元数据字段；syslog 文本日志格式精简，只输出基础 syslog 格式内容|
...
```text
获取命令：sudo grep -F "student_id=2024010025" /var/log/syslog | tail -n 5
日志原文：2026-09-17T08:52:45.270254+08:00 hakiii-VMware-Virtual-Platform lab3_read[3640]: student_id=2024010025 name=樊子逸 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17T08:52:45.270254+08:00|
| Where 在哪里 |主机 hakiii‑VMware‑Virtual‑Platform|
| Who 谁 |标签lab3_read，logger 工具写入；正文标识学号 2024010025、姓名樊子逸|
| What 做了什么 |使用 logger 向本机日志系统写入一条测试消息|
| Result 结果如何 |正文标记result=success；journal 与 syslog 均可查到该记录，写入生效|

**用一两句话解释这个事件：**

> 填写：在 hakiii‑VMware‑Virtual‑Platform 主机上，logger 生成一条测试日志。result=success只是写入的文本标记，不是系统校验结果；该消息同时被 journald 与 rsyslog 保存。
...
日志原文：9月 17 08:50:06 hakiii-VMware-Virtual-Platform sshd[3550]: Failed password for hakiii from 192.168.122.1 port 64486 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9 月 17 08:50:06|
| Where 在哪里 |主机 hakiii‑VMware‑Virtual‑Platform|
| Who 谁 |sshd 服务进程记录；尝试登录账号 hakiii；来源客户端 IP 192.168.122.1|
| What 做了什么 |客户端发起 SSH 密码登录尝试|
| Result 结果如何 |Failed password，本次密码认证失败|

**用一两句话解释这个事件：**

> 填写：外部客户端对 hakiii 账号发起 SSH 密码登录尝试，日志记录本次密码认证失败。仅该日志只能看到认证结果，无法确定实际操作者身份。
...
| 问题 | 本例怎样填写 |
| :--- | :--- |
| When 什么时候 |2026‑09‑17 08:39:11|
| Where 在哪里 | 取自本机 Ubuntu 虚拟机的/var/log/dpkg.log|
| Who 谁 |	dpkg 软件包管理工具；该日志未记录执行操作的用户账号|
| What 做了什么 |记录 python3.12‑minimal 软件包的状态变更|
| Result 结果如何 |status installed，软件包已安装|
...
```text
实际日志来源（使用替代来源时说明原因）：/var/log/dpkg.log，htop安装失败，使用系统已存在软件包安装记录
获取命令：grep "install" /var/log/dpkg.log | tail -n 10
日志原文：2026-09-17 08:39:11 status installed python3.12‑minimal:amd64 3.12.3‑1ubuntu0.17
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026‑09‑17 08:39:11；该日志未提供时区|
| Where 在哪里 |该日志未提供主机名；取自本机 Ubuntu 虚拟机的/var/log/dpkg.log|
| Who 谁 |dpkg 软件包管理工具；该日志未记录执行操作的用户账号|
| What 做了什么 |记录 python3.12‑minimal 软件包的状态变更|
| Result 结果如何 |status installed，软件包已安装|
...
> 填写：dpkg 包管理器记录软件包安装状态。该日志只记录软件包本身状态，不会记录是哪个用户执行的安装操作。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd‑journald 收集系统、程序产生的日志消息，保存为二进制 journal，通过 journalctl 读取；rsyslog 接收 journald 转发的 syslog 格式消息，写入 syslog、auth.log 等文本文件。同一条消息先被 journald 保存，journald 再将消息转发给 rsyslog，因此 journal 和文本日志可以同时保存同一条事件。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password可以证明对应 IP 对该账号做过密码登录尝试，并且密码校验失败。但不能确定操作者真实身份，不能断定是暴力破解，也不能直接证明账号已经遭到入侵。
