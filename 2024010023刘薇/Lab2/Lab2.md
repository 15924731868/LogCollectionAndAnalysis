...
> 记录：Ubuntu 的日期和时间为 __四 2026-09-10 09:09:10 CST____；时区为 ___Asia/Shanghai___；时间同步状态为 ____yes__。
...
> 记录：`/var/log/auth.log` 是否存在？____存在__；实际输出：____auth.log exists__。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ___192.168.109.129___。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 __0.0.0.0:22 和 [::]:22____。
...
> 记录：当前目录为 _/home/lw；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：`/var/log/journal/` 的检查结果为 ___不存在___；`/run/log/journal/` 的检查结果为 ___存在___。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog |记录系统通用运行日志，各类服务信息 |文本文件 |tail / less / cat |
|/var/log/auth.log |记录用户登录、SSH、sudo 等身份认证事件 |文本文件 |tail / less / cat |
| /var/log/wtmp|记录用户成功登录与注销历史 |二进制文件 |last |
|/var/log/btmp |/var/log/btmp |二进制文件 |lastb |
|/run/log/journal | systemd-journald 存放系统日志的目录|目录 |journalctl |
...
> 记录：选取的一条日志来自哪个程序或服务？__sshd（SSH 服务）____；记录了什么事件？__接受来自 192.168.109.1 的 lw 用户密码，成功建立 SSH 登录会话____。
...
> 记录：实际查询起点为 __2026-09-10 09:00:00____；终点为 _2026-09-10 10:40:00_____；观察到的事件或无记录情况为 ___包含 CRON 定时任务、sshd 用户 SSH 登录、systemd 系统服务启停等日志记录___。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 ___systemd-resolved[648]: Using degraded feature set TCP instead of UDP for DNS server 192.168.109.2.___。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制格式的日志文件，cat 只能读取纯文本，直接读取会输出乱码。读取 /var/log/wtmp 使用 last 命令，读取 /var/log/btmp 使用 lastb 命令。
