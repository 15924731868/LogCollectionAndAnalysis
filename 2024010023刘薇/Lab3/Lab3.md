...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 ___192.168.109.129___。
...
> 记录：本机主机名为 __lw-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| 两处记录有哪些共同字段或正文 | 时间、标签lab3_read、正文student_id=2024010023 name=刘薇 action=write_test result=success完全一致|
| 两处输出的主要区别 |journal 输出自带进程号；syslog 文本日志格式更简洁，journal 字段更丰富 |
