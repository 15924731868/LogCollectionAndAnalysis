...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 ___192.168.165.128__。
...
> 记录：本机主机名为 __user-VMware-Virtual-Platform__（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |9 月 17 日 08:20:41|user|192.168.165.1|登录成功|`journalctl -u ssh` 、`/var/log/auth.log`|
| 失败认证 |9 月 17 日 08:20:25|user|192.168.165.1|密码错误，认证失败|`journalctl -u ssh` 、`/var/log/auth.log`|
...
| 项目 | 你的记录 |
| :--- | :--- |
| journal 中是否查到 |是，成功查询到该条自定义日志|
| `/var/log/syslog` 中是否查到 |是，成功查询到该条自定义日志|
| 两处记录有哪些共同字段或正文 |标签`lab3_read`；正文：`student_id=2024010005 name=刘珈宁 action=write_test result=success`；时间、主机名完全一致|
| 两处输出的主要区别 |journalctl 输出为本地时区简化时间格式；syslog 输出是带时区偏移的 ISO 标准时间格式。journald 是二进制日志数据库，syslog 是纯文本日志文件。|
...
```text
获取命令：sudo grep -F "student_id=2024010005" /var/log/syslog | tail -n 5

日志原文：2026-09-17T08:31:23.648680+08:00 user-VMware-VirtualPlatform lab3_read: student_id=2024010005 name=刘珈宁 action=write_test result=success

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 17 日 08:31:23.648680+08:00|
| Where 在哪里 |主机 user-VMware-VirtualPlatform|
| Who 谁 |自己指定的标签 lab3_read；正文标识本人姓名为刘珈宁，学号 2024010005|
| What 做了什么 |使用 logger 命令向本机日志系统写入一条 write_test 测试消息|
| Result 结果如何 |正文写明 result=success，这是人为写入的测试标记；journald 和 syslog 两处查询均成功查到这条日志|
...
| 4W1R问题 | 本例怎样填写 |
| --- | --- |
| When   什么时候 | 2026年09月17日 08:31:23.648680+08:00 |
| Where   在哪里 | 主机 user-VMware-VirtualPlatform |
| Who   谁 | 自己指定的标签 lab3_read；正文标识本人姓名为刘珈宁，学号2024010005 |
| What   做了什么 | 使用logger命令向本机日志系统写入一条write_test测试消息 |
| Result   结果如何 | 正文写明result=success，这是人为写入的测试标记；journald和syslog两处查询均成功查到这条日志 |

## 获取命令：

```
sudo grep -F "student_id=2024010005" /var/log/syslog | tail -n 5
```

## 日志原文：

```
2026-09-17T08:31:23.648680+08:00 user-VMware-VirtualPlatform lab3_read: student_id=2024010005 name=刘珈宁 action=write_test result=success
```

## 人话解释（直接粘贴）

2026年09月17日08:31:23，本人在user-VMware-VirtualPlatform主机上使用logger命令写入一条学号2024010005、姓名刘珈宁的测试日志消息，正文标记操作结果为success。
...
日志原文：2026-09-17T08:20:41.175093+08:00 user-VMware-Virtual-Platform sshd[4533]: Accepted password for user from 192.168.165.1 port 52463 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17T08:20:41.175093+08:00|
| Where 在哪里 |主机 user-VMware-Virtual-Platform，来源 IP：192.168.165.1|
| Who 谁 |记录程序：sshd；尝试登录账号：user|
| What 做了什么 |客户端从 192.168.165.1 发起 SSH 密码登录请求|
| Result 结果如何 |密码认证成功，SSH 登录通过（Accepted password）|

**用一两句话解释这个事件：**

> 填写：2026 年 09 月 17 日 08:20，来自 192.168.165.1 的客户端向虚拟机发起 SSH 登录，账号 user 的密码认证成功，完成 SSH 远程登录。
...
日志原文：2026-09-17 08:45:23 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 17 日 08:45:23|
| Where 在哪里 |该日志取自 Ubuntu 虚拟机的 `/var/log/dpkg.log`，日志未直接给出主机名|
| Who 谁 |记录工具为 dpkg；日志没有记录执行操作的用户账号|
| What 做了什么 |记录 htop 软件包的安装状态变更|
| Result 结果如何 |状态为 installed，代表 htop 软件包安装完成；仅靠本条日志无法确认是否为首次安装|

**用一两句话解释这个事件：**

> 填写：2026 年 09 月 17 日 08:45，dpkg 包管理器记录 htop 软件包状态变为 installed，htop（amd64 架构，版本 3.3.0-4build1）成功安装到虚拟机。
---
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 自带日志服务，收集系统所有日志并保存到 journal 数据库；rsyslog 是传统日志服务，接收日志并写入文本文件（如 syslog）。journald 会把收到的日志转发给 rsyslog，所以同一条日志会被两者分别存储，实现同时保留。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password 能证明有来自对应 IP 的客户端尝试 SSH 登录，并且本次密码输入错误、认证失败。但它**不能证明这个 IP 对应的人就是登录尝试者**，来源 IP 可以伪造，无法单凭日志确定实际操作人。
