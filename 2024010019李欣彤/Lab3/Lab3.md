...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.250.128__。
...
> 记录：本机主机名为 __lxt-VMware-Virtual-Platform__（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-17 08:38:49 |lxt | 192.168.250.1| Accepted password|journalctl/var/log/auth.log |
| 失败认证 | 2026-09-17 08:38:43|lxt | 192.168.250.1|Failed password |journalctl/var/log/auth.log |
...
| 两处记录有哪些共同字段或正文 | 标签`lab3_read`、学号`student_id=2024010019`、姓名`name=lxt`、动作`action=write_test`、结果`result=success`，事件发生时间一致|
| 两处输出的主要区别 |journalctl 输出：主机名、进程号，格式简洁；syslog 输出：时间带时区，日志格式更长，包含完整时间戳，没有进程号 |
...
```text
获取命令：logger -p user.notice -t lab3_read "student_id=2024010019 name=lxt action=write_test result=success"
日志原文：Sep 17 08:54:31 lxt-VMware-Virtual-Platform lab3_read[5044]: student_id=2024010019 name=lxt action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月17日 08:54:31 |
| Where 在哪里 |主机 `lxt-VMware-Virtual-Platform` |
| Who 谁 |标签 `lab3_read`（进程号 5044）写入；正文中标识的本人姓名为 lxt，学号 2024010019 |
| What 做了什么 |向本机日志系统写入一条 `write_test` 测试消息 |
| Result 结果如何 |正文写明 `result=success`，结合 journalctl 与 /var/log/syslog 两处查询均能查到这条记录，印证日志写入成功 |

**用一两句话解释这个事件：**

> 填写：Sep 17 08:54:31，我在虚拟机主机上使用 logger 命令写入自定义测试日志，日志记录成功保存，在 journal 和 syslog 中都可以查询到。
...
日志原文：Sep 17 08:38:43 lxt-VMware-Virtual-Platform sshd[4737]: Failed password for lxt from 192.168.250.1 port 52964 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |Sep 17 08:38:43；该显示未提供年份和时区 |
| Where 在哪里 | 主机 `lxt-VMware-Virtual-Platform`|
| Who 谁 |记录程序为`sshd`；尝试登录的账号是 lxt，来源客户端 IP：192.168.250.1 |
| What 做了什么 | 客户端通过 SSH 协议尝试远程登录本机，输入密码进行身份认证|
| Result 结果如何 |`Failed password`，本次 SSH 密码认证失败 |

**用一两句话解释这个事件：**

> 填写：Sep 17 08:38:43，来自 192.168.250.1 的客户端使用账号 lxt 发起 SSH 登录，密码输入错误，本次身份认证失败。
...
日志原文：2026-09-17 12:38:40 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 17 日 12:38:40；该日志未提供时区 |
| Where 在哪里 | 该日志未提供主机名；可以另注 “从本人 Ubuntu 虚拟机的 `/var/log/dpkg.log` 取得”|
| Who 谁 | 记录工具为 `dpkg`；该日志未提供执行操作的用户账号|
| What 做了什么 |记录 `htop` 软件包的状态 |
| Result 结果如何 |状态为 `installed`，即已安装；仅凭这一行不能判断此前执行的是首次安装还是升级 |
...
> 填写：9 月 17 日 12:38:40，软件包管理工具 dpkg 记录了 htop 软件包为已安装状态，这一行日志没有记录是哪个用户执行的安装操作。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是系统日志服务，收集内核、程序输出的日志，存放在二进制 journal 日志；rsyslog 是传统 syslog 服务，接收日志并保存为文本格式日志文件。systemd-journald 收到日志消息后，会转发一份给 rsyslog，因此同一条日志会被两个服务分别保存。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：`Failed password` 可以证明有人尝试 SSH 登录，输入的密码错误；**不能证明攻击者已经拿到账号密码，也不能证明登录账号真实存在**，有可能是暴力扫描尝试。
