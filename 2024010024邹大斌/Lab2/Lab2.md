...
> 记录：journald 的实际运行状态为 __active___。
...
> 记录：Ubuntu 的日期和时间为 __2026-09-16 18:59:53____；时区为 __Asia/Shanghai____；时间同步状态为 ___yes___。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ___192.168.184.128___。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 __0.0.0.0:22 [::]:22____。
...
> 记录：SSH 登录后的主机名为 __hu-VMware-Virtual-Platform____。
...
> 记录：当前目录为 __/home/hu____；是否确认已通过 Git Bash 登录 Ubuntu？___是___。
...
> 记录：`/var/log/journal/` 的检查结果为 __存在____；`/run/log/journal/` 的检查结果为 ___存在___。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 __Speculative Return Stack Overflow: IBPB-extending microcode not applied!____。
...
> 填写： /var/log/wtmp  和  /var/log/btmp  是二进制格式文件，不是普通文本， cat  读取只会显示乱码。读取wtmp用 last 命令，读取btmp用 lastb 命令。
