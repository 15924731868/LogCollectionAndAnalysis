...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 _192.168.16.100_____。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是系统日志服务，接收来自内核、服务、程序产生的日志，存储为二进制 journal 日志；rsyslog 是传统日志服务，接收日志消息并写入 /var/log/ 下文本日志文件。logger 发送日志消息时，消息会同时交给 systemd-journald，再转发给 rsyslog，因此同一条消息会同时保存在 journal 二进制日志和 syslog 文本日志两个位置。
