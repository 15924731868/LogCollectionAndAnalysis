...
> 记录：Ubuntu 的日期和时间为 _2026-09-16 21:xx:xx_____；时区为 _Asia/Shanghai_____；时间同步状态为 __systemd-timesyncd.service active____。
...
> 记录：`/var/log/syslog` 是否存在？__存在____；实际输出：_syslog exists_____。
...
> 记录：`/var/log/auth.log` 是否存在？__存在____；实际输出：_auth.log exists_____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 _192.168.249.130_____。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 _0.0.0.0:22, [::]:22_____。
...
> 记录：SSH 登录后的主机名为 __ubuntu-Lab____。
...
> 记录：当前目录为 ___/home/student___；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
> 记录：选取的一条日志来自哪个程序或服务？__rsyslog____；记录了什么事件？__rsyslog 服务接收系统日志消息____。
...
> 记录：实际查询起点为 __2026-09-16 20:00:00____；终点为 __2026-09-16 22:00:00____；观察到的事件或无记录情况为 __观测到 ssh 登录、系统服务运行相关日志____。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 __检测到系统警告类消息____。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 属于二进制日志文件，不能直接使用cat读取。读取wtmp使用last命令，读取btmp使用lastb命令。
