...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 192.168.55.129。
...
> 记录：本机主机名为 qs-VMware-Virtual-Platform（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 | 2026-09-24 08:06:45 | qs | 192.168.55.1 | Accepted password | journal（sshd）与 /var/log/auth.log |
| 失败认证 | 2026-09-24 08:06:34 | qs | 192.168.55.1 | Failed password | journal（sshd）与 /var/log/auth.log |
...
| 两处记录有哪些共同字段或正文 | 学号 student_id=2024010020、姓名 name=朱勇章、动作 action=write_test、结果 result=success，正文完全一致 |
| 两处输出的主要区别 | journal 显示 `9月 24 08:10:04` 格式、带进程号 `lab3_read[3828]`；syslog 显示 ISO 时间 `2026-09-24T08:10:04.848699+08:00`、无进程号 |
...
```text
获取命令：sudo grep -F "student_id=2024010020" /var/log/syslog | tail -n 5
日志原文：2026-09-24T08:10:04.848699+08:00 qs-VMware-Virtual-Platform lab3_read: student_id=2024010020 name=朱勇章 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026 年 9 月 24 日 08:10:04（+08:00，含年份和时区） |
| Where 在哪里 | 主机 qs-VMware-Virtual-Platform |
| Who 谁 | 自己指定的标签 lab3_read（进程号 3828）写入；正文中标识的本人姓名为朱勇章，学号 2024010020 |
| What 做了什么 | 向本机日志系统写入一条 write_test 测试消息 |
| Result 结果如何 | 正文写明 result=success，这是自己写入的测试标记，结合两处查询（journal 与 syslog）均能查到来印证 |

**用一两句话解释这个事件：**

> 2026 年 9 月 24 日 08:10:04，本人在 qs-VMware-Virtual-Platform 主机上用 logger 写入一条学号为 2024010020 的测试消息，正文标记结果为 success，journal 与 /var/log/syslog 两处均查到了同一条记录。
...
日志原文：2026-09-24T08:06:34.113269+08:00 qs-VMware-Virtual-Platform sshd[3555]: Failed password for qs from 192.168.55.1 port 52305 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026 年 9 月 24 日 08:06:34（+08:00，含年份和时区） |
| Where 在哪里 | 主机 qs-VMware-Virtual-Platform |
| Who 谁 | 记录程序为 sshd（进程号 3555）；尝试登录的账号为 qs；来源 IP 为 192.168.55.1 |
| What 做了什么 | 来自 192.168.55.1 的连接尝试用 qs 账号和密码登录 SSH |
| Result 结果如何 | 这次密码认证失败，依据是 Failed password |

**用一两句话解释这个事件：**

> 2026 年 9 月 24 日 08:06:34，来自 192.168.55.1 的连接尝试用 qs 账号和密码登录本机 SSH，密码认证失败。
...
```text
实际日志来源（使用替代来源时说明原因）：/var/log/dpkg.log（htop 安装成功，使用正常来源）
获取命令：grep "htop" /var/log/dpkg.log | tail -n 10
日志原文：2026-09-24 08:16:19 status installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026 年 9 月 24 日 08:16:19；该日志未提供时区 |
...
> 2026 年 9 月 24 日 08:16:19，软件包管理工具 dpkg 记录了 htop（3.3.0-4build1）已安装的状态。这一行没有说明是哪位用户执行的操作。
...
> systemd-journald 是系统日志守护进程，把日志写入二进制 journal 数据库，可用 journalctl 查询；rsyslog 把收到的日志消息写入 /var/log/ 下的文本文件（如 syslog、auth.log）。同一条日志可以同时被两者记录：logger 发送的消息先进入系统日志管道，systemd-journald 和 rsyslog 各自监听并分别保存，所以 3.2 节中同一条 lab3_read 消息在 journalctl 和 /var/log/syslog 两处都能查到。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

> Failed password 能证明：本次 SSH 密码认证失败（账号 qs、来源 192.168.55.1、时间 08:06:34）。它不能证明：是谁在操作（来源 IP 不能单独证明实际操作者身份）、失败的具体原因（密码输错还是其他）、以及是否发生了入侵或恶意攻击。
