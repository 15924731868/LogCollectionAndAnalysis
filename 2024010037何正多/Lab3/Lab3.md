...
> 记录：本次使用的 Ubuntu 用户名为 __hzd____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.222.128 ____。
...
> 记录：本机主机名为 __hzd-VMware-Virtual-Platform
____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 | 9月 17 08:55:30 |hzd | 192.168.222.128|  Accepted password for hzd from 192.168.222.1 port 57102 ssh2|journalctl -u ssh |
| 失败认证 |9月 17 08:55:23  |student |192.168.222.128 | Failed password for hzd from 192.168.222.1 port 57102 ssh2 | journalctl -u ssh|
...
| 两处记录有哪些共同字段或正文 |① 时间戳（同一秒级时间，均为系统本地时间）；② 主机名 VMware-Virtual-Platform；③ 日志标识/标签 lab3_read[PID]；④ 正文完全一致：student_id=2024010037 name=何正多 action=write_test result=success。 |
| 两处输出的主要区别 |1. 格式与字段丰富度：journalctl 是 systemd 结构化日志，除可见行外还带 _PID、_UID、_SYSTEMD_UNIT、PRIORITY、_TRANSPORT、__MONOTONIC_TIMESTAMP 等隐藏元数据（journalctl -o verbose 可见）；/var/log/syslog 是 rsyslog 写的纯文本单行，只有"时间 主机 标签[PID]: 正文"，没有优先级/单元等字段。
2. 检索方式：journal 支持 -t、-u、-p、--since/--until 等原生过滤；syslog 只能靠 grep/awk 文本匹配。
3. 存储与生命周期：syslog 是持久化明文文件，可直接 cat/tail；journal 默认在 /run/log/journal（内存中，重启即丢），只有存在 /var/log/journal 时才持久保存。
4. 输出范围：journalctl -t lab3_read 直接只給目标标签的记录；grep syslog 的结果混在全系统日志里，需要用标签过滤并注意权限（sudo）。 |
...
```text
获取命令：sudo grep "student_id=2024010037" /var/log/syslog | tail -n 5
日志原文：2026-09-17T09:23:37hzd-VMware-Virtual-Platform lab3_read[2310]: student_id=2024010037 name=何正多 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17T09:23:37 |
| Where 在哪里 | 主机 hzd-VMware-Virtual-Platform，标签 lab3_read，进程号 2310|
| Who 谁 |学号 student_id=2024010037，姓名 name=何正多 |
| What 做了什么 |action=write_test — 执行一次写入测试操作 |
| Result 结果如何 |result=success — 操作成功 |

**用一两句话解释这个事件：**

> 填写：2026-09-17T09:23:37，本人在 ubuntu 主机上用 logger 写入一条学号为 2024010037 的测试消息，正文标记结果为 success。
...
日志原文：[sudo] hzd 的密码：
2026-09-17T08:52:30.137528+08:00 hzd-VMware-Virtual-Platform sshd[3473]: Failed password for invalid user student from 192.168.222.1 port 54096 ssh2
2026-09-17T08:52:59.060407+08:00 hzd-VMware-Virtual-Platform sshd[3473]: Failed password for invalid user student from 192.168.222.1 port 54096 ssh2
2026-09-17T08:53:09.660020+08:00 hzd-VMware-Virtual-Platform sshd[3473]: Failed password for invalid user student from 192.168.222.1 port 54096 ssh2
2026-09-17T08:55:23.461612+08:00 hzd-VMware-Virtual-Platform sshd[3490]: Failed password for hzd from 192.168.222.1 port 57102 ssh2
2026-09-17T08:55:30.767557+08:00 hzd-VMware-Virtual-Platform sshd[3490]: Accepted password for hzd from 192.168.222.1 port 57102 ssh2
2026-09-17T08:56:10.375410+08:00 hzd-VMware-Virtual-Platform sudo: hzd : TTY=pts/1 ; PWD=/home/hzd ; USER=root ; COMMAND=/usr/bin/journalctl -u ssh --since '15 minutes ago' --no-pager
2026-09-17T08:56:10.376710+08:00 hzd-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session opened for user root(uid=0) by hzd(uid=1000)
2026-09-17T08:56:10.399498+08:00 hzd-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session closed for user root
2026-09-17T08:56:21.137907+08:00 hzd-VMware-Virtual-Platform sudo: hzd : TTY=pts/1 ; PWD=/home/hzd ; USER=root ; COMMAND=/usr/bin/grep -E 'Accepted|Failed password' /var/log/auth.log
2026-09-17T08:56:21.138347+08:00 hzd-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session opened for user root(uid=0) by hzd(uid=1000)
2026-09-17T08:56:21.141138+08:00 hzd-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session closed for user root
2026-09-17T09:24:47.704969+08:00 hzd-VMware-Virtual-Platform sudo: hzd : TTY=pts/1 ; PWD=/home/hzd ; USER=root ; COMMAND=/usr/bin/journalctl -t lab3_read --since '5 minutes ago' --no-pager
2026-09-17T09:24:47.707163+08:00 hzd-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session opened for user root(uid=0) by hzd(uid=1000)
2026-09-17T09:24:47.727872+08:00 hzd-VMware-Virtual-Platform sudo: pam_unix(sudo:session): session closed for user root
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026-09-17 08:52~09:52 |
| Where 在哪里 |hzd-VMware-Virtual-Platform 虚拟机，来源 IP：192.168.222.1 |
| Who 谁 |用户 hzd；SSH 登录来源客户端 192.168.222.1 |
| What 做了什么 |1. 客户端尝试 ssh 登录，先用不存在的 student 账号多次密码失败；2. 使用 hzd 账号先密码错误，之后密码验证成功登录；3. hzd 用户多次执行 sudo，使用 root 权限查看系统认证日志、syslog 日志 |
| Result 结果如何 |SSH：无效用户 student 登录全部失败；hzd 账号最终 SSH 登录成功；sudo 命令全部成功执行，成功打开 root 会话，命令执行完毕关闭 root 会话 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日，来自 192.168.222.1 的客户端尝试 SSH 登录虚拟机，使用 student 无效用户多次密码失败，后续 hzd 用户 SSH 登录成功，并且 hzd 多次执行 sudo 命令，以 root 权限查询系统日志。
...
```text
实际日志来源（使用替代来源时说明原因）：
获取命令：sudo journalctl -k -b -n 30 --no-pager
日志原文：2026-09-17 10:56:54 install htop:amd64 <none> 3.3.0-4build1
2026-09-17 10:56:54 status half-installed htop:amd64 3.3.0-4build1
2026-09-17 10:56:54 status unpacked htop:amd64 3.3.0-4build1
2026-09-17 10:56:54 configure htop:amd64 3.3.0-4build1 <none>
2026-09-17 10:56:54 status unpacked htop:amd64 3.3.0-4build1
2026-09-17 10:56:54 status half-configured htop:amd64 3.3.0-4build1
2026-09-17 10:56:54 status installed htop:amd64 3.3.0-4build1

```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 | 2026-09-17 10:56:54|
| Where 在哪里 |hzd-VMware-Virtual-Platform 虚拟机系统内，dpkg 包管理器日志 |
| Who 谁 |系统 dpkg 软件包管理程序（用户 hzd 发起 htop 安装） |
| What 做了什么 | 执行 htop 软件包安装，依次完成包解压、配置，更新软件状态|
| Result 结果如何 | htop 3.3.0-4build1 版本安装成功，最终状态标记为 installed|

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 17 日 10:56:54，在虚拟机中通过 dpkg 完成 htop 软件包安装，经历解压、配置等阶段，最终 htop 成功安装到系统。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald：是 systemd 自带日志服务，接收内核、服务、应用输出的日志，以二进制格式存放在 journal 数据库，使用`journalctl`命令读取日志，保存丰富元数据。rsyslog：传统系统日志服务，接收系统日志消息，将日志整理为文本格式，输出到`/var/log/`目录下各类日志文件（如 auth.log、syslog）。原因：系统产生的日志消息会**同时发给 systemd-journald 和 rsyslog**两个服务。journald 存入二进制 journal 数据库，rsyslog 写入文本日志文件，两个独立存储，因此同一条日志可以被两者同时保存。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：能证明：来自对应 IP 的客户端发起 SSH 登录尝试，提交了密码，服务器校验密码失败。例如日志中`Failed password for invalid user student from 192.168.222.1`，证明该 IP 尝试用 student 账号登录，密码校验不通过；也有 hzd 用户第一次密码输入错误的记录。不能证明：不能证明客户端操作者是谁；不能证明客户端真实身份；不能证明密码是用户手动输入还是爆破脚本提交；不能证明账号本身是否真实存在（日志里 invalid user 代表账号不存在也会报 Failed password）；仅能证明密码校验失败，无法确定失败原因是密码错误还是用户名无效。
