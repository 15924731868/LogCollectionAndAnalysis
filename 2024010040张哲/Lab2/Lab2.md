...
> 记录：Ubuntu 的日期和时间为 ___2026-09-14 13:13:xx___；时区为 _Asia/Shanghai_____；时间同步状态为 ___systemd-timesyncd running___。
...
> 记录：`/var/log/syslog` 是否存在？__存在____；实际输出：__syslog exists。____。
...
> 记录：`/var/log/auth.log` 是否存在？____存在__；实际输出：__auth.log exists____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ___192.168.16.130___。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 __0.0.0.0:22____。
...
> 记录：SSH 登录后的主机名为 __hcl-VMware-Virtual-Platform____。
...
> 记录：当前目录为 _/home/hcl_____；是否确认已通过 Git Bash 登录 Ubuntu？_是_____。
...
> 记录：`/var/log/journal/` 的检查结果为 __存在目录____；`/run/log/journal/` 的检查结果为 ____存在目录__。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog|	系统综合服务日志|	文本	tail、grep|
|/var/log/auth.log|	用户登录、sudo 认证审计	文本	|tail、grep|
|/var/log/kern.log	|内核、硬件驱动相关日志	文本 | tail|
|/var/log/journal/|	systemd-journald 持久化日志存储	目录（内部二进制文件）|	journalctl|
|/var/log/wtmp|	用户登录 / 注销历史记录	二进制	|last|
...
> 记录：选取的一条日志来自哪个程序或服务？___systemd___；记录了什么事件？____系统服务重新加载配置。__。
...
> 记录：实际查询起点为 _2026-09-14 13:00:00_____；终点为 _2026-09-14 13:01:00_____；观察到的事件或无记录情况为 _____记录了 sysstat、fwupd、蓝牙等系统服务启停事件。_。
...
> 记录：是否查到匹配日志？__是____；其中一条的内容或无记录提示为 _kernel: CPUID marked event: 'cpu cycles' unavailable，内核警告，虚拟机硬件事件。_____。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制数据库文件，不是普通文本，cat只能打印乱码。读取wtmp使用last命令，读取btmp（失败登录）使用sudo lastb。
